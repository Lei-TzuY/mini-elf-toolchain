...
pub enum OrderedLinkInputError {
    ...
    InvalidArchiveIndex {
        input_index: usize,
        source: ArchiveSymbolIndexError,
    },
    MissingArchiveIndex {
        input_index: usize,
    },
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidObject {
                input_index,
                source,
            } => write!(f, "input {input_index} is not a valid ET_REL object: {source}"),
            Self::ObjectSymbols {
                input_index,
                object_index,
                source,
                ...
                f,
                "cannot read symbols from input {input_index} object {object_index}: {source}"
            ),
            Self::UnsupportedBinding {
                ...
                f,
                "input {input_index} object {object_index} symbol {symbol_index} in table section {table_section_index} uses unsupported binding {binding}"
            ),
            Self::InvalidArchive {
                input_index,
                source,
            } => write!(f, "input {input_index} is not a valid archive: {source}"),
            Self::InvalidArchiveIndex {
                input_index,
                source,
            } => write!(f, "input {input_index} has an invalid archive index: {source}"),
            Self::MissingArchiveIndex { input_index } => write!(
                f,
            ...
            Self::ArchiveExtraction {
                ...
                f,
                "cannot lazily extract archive input {input_index}: {source}"
            ),
        }
    }
}
    ...
    for (input_index, input) in inputs.iter().copied().enumerate() {
        match input {
            OrderedLinkInput::Object(file) => {
                ...
                let object = LinkerInputObject::parse(object_index, file).map_err(|source| {
                    OrderedLinkInputError::InvalidObject {
                        input_index,
                        source,
                    }
                })?;
            ...
            OrderedLinkInput::Archive(file) => {
                ...
                let index = parse_archive_symbol_index(&archive)
                    .map_err(|source| OrderedLinkInputError::InvalidArchiveIndex {
                        input_index,
                        source,
                    })?
                ...
                .map_err(|source| OrderedLinkInputError::ArchiveExtraction {
                    input_index,
                    source,
                })?;
...
fn update_symbol_state(
    ...
    for table in tables {
        let named = named_symbols_from_table(file, &object.object.sections, table, object_index)
            .map_err(|source| OrderedLinkInputError::ObjectSymbols {
                input_index,
                object_index,
                source,
            })?;
        for candidate in named {
            ...
            if binding == STB_LOCAL || candidate.name.is_empty() {
                continue;
            }
            ...
            if candidate.symbol.section_index == SHN_UNDEF {
                if binding == STB_GLOBAL && !defined.contains(&name) {
