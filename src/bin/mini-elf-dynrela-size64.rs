...
fn run<I: Iterator<Item = OsString>>(args: I) -> Result<String, String> {
    ...
    if inputs.is_empty() {
        return Err("usage: mini-elf-dynrela-size64 <input>...".into());
    }
...
fn inspect(bytes: &[u8]) -> Result<String, String> {
    ...
    let mut out = format!(
        "Validated R_X86_64_SIZE64 relocations: entries={} symbols={symbol_count}\n",
        relasz / 24
    );
    ...
    for (index, entry) in rela_bytes.chunks_exact(24).enumerate() {
        ...
        if info as u32 != R_X86_64_SIZE64 {
            continue;
        }
        ...
        if section_index == 0 {
            ...
            if section_index == SHN_ABS {
                return Err(format!("R_X86_64_SIZE64 relocation {index} references an absolute symbol; absolute-symbol semantics are outside this bounded slice"));
            }
    ...
    if found == 0 {
        return Err("DT_RELA contains no R_X86_64_SIZE64 relocations".into());
    }
    Ok(out)
}
...
fn dynamic_symbol_count(
    ...
    let address = gnu_hash.ok_or_else(|| {
        "R_X86_64_SIZE64 validation requires DT_HASH or DT_GNU_HASH to bound DT_SYMTAB".to_owned()
    })?;
