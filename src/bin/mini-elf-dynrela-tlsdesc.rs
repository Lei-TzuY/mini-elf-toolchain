...
fn usage() -> String {
    "usage: mini-elf-dynrela-tlsdesc --load-bias <address> <input>...".into()
}
...
fn inspect(file: &[u8], bias: u64) -> Result<String, String> {
    ...
    if file.len() < 64 || u16at(file, 16) != ET_DYN {
        return Err("mini-elf-dynrela-tlsdesc requires an ET_DYN image".into());
    }
    ...
    for (idx, e) in relas.chunks_exact(RELAENT).enumerate() {
        let off = u64at(e, 0);
        let info = u64at(e, 8);
        if info as u32 != R_X86_64_TLSDESC {
            continue;
        }
        let si = info >> 32;
        if si == 0 || si >= count {
            return Err(format!("R_X86_64_TLSDESC relocation {idx} references invalid dynamic symbol index {si} (count {count})"));
        }
        memory_range(
            &ph,
            off,
            16,
            PF_W,
            &format!("R_X86_64_TLSDESC relocation {idx} descriptor target"),
        )?;
        ...
        let value = u64at(s, 8);
        if sh == SHN_UNDEF {
            return Err(format!("R_X86_64_TLSDESC relocation {idx} references undefined TLS symbol; dependency lookup is outside this bounded slice"));
        }
        if sh == SHN_ABS || typ != STT_TLS {
            return Err(format!(
                "R_X86_64_TLSDESC relocation {idx} requires a defined STT_TLS symbol"
            ));
        }
        let add = i64at(e, 16);
        let tls_offset = (value as i128) + (add as i128);
        let tls_offset = u64::try_from(tls_offset).map_err(|_| {
            format!("R_X86_64_TLSDESC relocation {idx} TLS offset does not fit unsigned 64-bit")
        })?;
        let rt = bias.checked_add(off).ok_or_else(|| {
            format!("R_X86_64_TLSDESC relocation {idx} runtime descriptor target overflows u64")
        })?;
        rt.checked_add(15).ok_or_else(|| {
            format!("R_X86_64_TLSDESC relocation {idx} runtime descriptor range overflows u64")
        })?;
    ...
    if found == 0 {
        return Err("DT_RELA contains no R_X86_64_TLSDESC relocations".into());
    }
    Ok(out)
}
