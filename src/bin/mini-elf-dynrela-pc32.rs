...
fn run<I: Iterator<Item = OsString>>(args: I) -> Result<String, String> {
    ...
    if inputs.is_empty() {
        return Err("usage: mini-elf-dynrela-pc32 <input>...".into());
    }
    let mut out = Vec::new();
    for p in &inputs {
...
fn inspect(b: &[u8]) -> Result<String, String> {
    ...
    for (i, e) in db.chunks_exact(16).enumerate() {
        ...
        if slot.replace(v).is_some() {
            return Err(format!("duplicate dynamic tag at entry {i}"));
        }
    }
    if !term {
        return Err("PT_DYNAMIC lacks DT_NULL".into());
    }
    let sym = req(sym, "DT_SYMTAB")?;
    let syment = req(syment, "DT_SYMENT")?;
    let strtab = req(strtab, "DT_STRTAB")?;
    let strsz = req(strsz, "DT_STRSZ")?;
    let rela = req(rela, "DT_RELA")?;
    let relasz = req(relasz, "DT_RELASZ")?;
    let relaent = req(relaent, "DT_RELAENT")?;
    if syment != 24 || relaent != 24 || relasz % 24 != 0 {
        return Err("unsupported dynamic table entry size".into());
    }
    let n = dynamic_symbol_count(b, &hs, hash, gnu_hash)?;
    if n == 0 {
    ...
    let sb = map(
        ...
        &hs,
        sym,
        n.checked_mul(24).ok_or("dynsym size overflow")?,
        0,
        "DT_SYMTAB",
    )?;
    let st = map(b, &hs, strtab, strsz, 0, "DT_STRTAB")?;
    let rb = map(b, &hs, rela, relasz, 0, "DT_RELA")?;
    let mut text = format!(
        "Validated R_X86_64_PC32 relocations: entries={} symbols={n}\n",
        relasz / 24
    );
    let mut found = 0;
    for (i, r) in rb.chunks_exact(24).enumerate() {
        let off = u64at(r, 0);
        let info = u64at(r, 8);
        if info as u32 != R_X86_64_PC32 {
            continue;
        }
        let si = info >> 32;
        if si == 0 || si >= n {
            return Err(format!(
                "R_X86_64_PC32 relocation {i} has invalid symbol index {si}"
            ));
        }
        mem(&hs, off, 4, PF_W, "PC32 target")?;
        let so = usize::try_from(si.checked_mul(24).ok_or("symbol offset overflow")?)
            .map_err(|_| "symbol offset too large")?;
        let s = &sb[so..so + 24];
        let no = u32at(s, 0) as usize;
        let sh = u16at(s, 6);
        let sv = u64at(s, 8);
        if no >= st.len() {
            return Err(format!(
        ...
        let name =
            std::str::from_utf8(&tail[..z]).map_err(|_| "dynamic symbol name is not UTF-8")?;
        let a = i64at(r, 16);
        if sh == 0 {
            ...
            i32::try_from(v)
                .map_err(|_| format!("R_X86_64_PC32 relocation {i} result does not fit i32"))?;
            text.push_str(&format!("  index={i} symbol={si}:{name} binding=same-image target={off:#x} addend={a} result={v}\n"));
        }
        found += 1;
    }
    if found == 0 {
        return Err("DT_RELA contains no R_X86_64_PC32 relocations".into());
    }
    Ok(text)
}
...
fn dynamic_symbol_count(
    ...
    if let Some(address) = hash {
        let header = map(b, hs, address, 8, 0, "DT_HASH header")?;
        return Ok(u64::from(u32at(header, 4)));
    }
    let address = gnu_hash.ok_or_else(|| {
        "R_X86_64_PC32 validation requires DT_HASH or DT_GNU_HASH to bound DT_SYMTAB".to_owned()
    })?;
    gnu_hash_symbol_count(b, hs, address)
}

fn gnu_hash_symbol_count(b: &[u8], hs: &[Phdr], address: u64) -> Result<u64, String> {
    let header = map(b, hs, address, 16, 0, "DT_GNU_HASH header")?;
    ...
    let prefix = map(b, hs, address, prefix_size, 0, "DT_GNU_HASH prefix")?;
...
fn phdrs(b: &[u8]) -> Result<Vec<Phdr>, String> {
    let off = usize::try_from(u64at(b, 32)).map_err(|_| "phoff too large")?;
    let ents = usize::from(u16at(b, 54));
    let n = usize::from(u16at(b, 56));
    if ents != 56 {
        return Err("unsupported program-header size".into());
    }
    let end = off
        .checked_add(n.checked_mul(ents).ok_or("program-header size overflow")?)
    ...
    for i in 0..n {
        ...
        if h.filesz > h.memsz {
            return Err(format!("program header {i} filesz exceeds memsz"));
        }
        file_range(b, h.off, h.filesz, "program header")?;
...
fn file_range<'a>(b: &'a [u8], off: u64, size: u64, label: &str) -> Result<&'a [u8], String> {
    let s = usize::try_from(off).map_err(|_| format!("{label} offset too large"))?;
    let w = usize::try_from(size).map_err(|_| format!("{label} size too large"))?;
    let e = s
        .checked_add(w)
        .ok_or_else(|| format!("{label} range overflow"))?;
    if e > b.len() {
        return Err(format!("{label} exceeds input"));
    }
    Ok(&b[s..e])
}
fn map<'a>(
    ...
    {
        let he = h.va + h.filesz;
        if va >= h.va && end <= he {
            return file_range(b, h.off + (va - h.va), size, label);
        }
    }
    Err(format!("{label} is not file-backed by PT_LOAD"))
}
fn mem(hs: &[Phdr], va: u64, size: u64, flags: u32, label: &str) -> Result<(), String> {
    let end = va
        .checked_add(size)
        .ok_or_else(|| format!("{label} address overflow"))?;
    for h in hs
        ...
        .filter(|h| h.kind == PT_LOAD && h.flags & flags == flags)
    {
        if va >= h.va && end <= h.va + h.memsz {
            return Ok(());
        }
    }
    Err(format!("{label} is not contained in matching PT_LOAD"))
}
fn req(v: Option<u64>, n: &str) -> Result<u64, String> {
    v.ok_or_else(|| format!("missing {n}"))
}
