    ...
    I: Iterator<Item = OsString>,
{
    ...
    if inputs.is_empty() {
        return Err(
            "usage: mini-elf-dynrela-relative64 --load-bias <address> <input>...".to_owned(),
        );
    }
    ...
    for input in &inputs {
        ...
        let report =
            inspect(header, &bytes, load_bias).map_err(|error| format!("{display}: {error}"))?;
...
fn inspect(header: Elf64Header, file: &[u8], load_bias: u64) -> Result<String, String> {
    ...
    let present = usize::from(rela.is_some())
        + usize::from(relasz.is_some())
        + usize::from(relaent.is_some());
    if present == 0 {
    ...
    if relaent != ELF64_RELA_SIZE {
        return Err(format!(
            "DT_RELAENT is {relaent}, expected {ELF64_RELA_SIZE} for ELF64"
        ));
    }
    if relasz % relaent != 0 {
        return Err(format!(
            "DT_RELASZ {relasz} is not a multiple of DT_RELAENT {relaent}"
        ));
    }
    let table_offset =
        map_virtual_file_range(&program_headers, file.len(), rela, relasz, "DT_RELA table")?;
    ...
    for index in 0..count {
        ...
        if symbol != 0 {
            return Err(format!(
                "DT_RELA entry {index} R_X86_64_RELATIVE64 uses non-zero symbol index {symbol}"
            ));
        }
    ...
    let mut output = format!(
        "DT_RELA contains {count} entries, {} R_X86_64_RELATIVE64 relocations:\n",
        found.len()
    );
...
fn dynamic_entries(headers: &[ProgramHeader], file: &[u8]) -> Result<Vec<DynamicEntry>, String> {
    let dynamic = headers
        .iter()
        .enumerate()
        .filter(|(_, h)| h.segment_type == PT_DYNAMIC)
        .collect::<Vec<_>>();
    if dynamic.len() > 1 {
        return Err(format!(
    ...
    let Some((segment_index, segment)) = dynamic.first().copied() else {
        return Ok(Vec::new());
    };
