    ...
    I: Iterator<Item = OsString>,
{
    ...
    if args.is_empty() || args[0] == "--help" || args[0] == "-h" {
        return if args.len() <= 1 {
            Ok("usage: mini-elf-gnuhash <input>...\n".to_owned())
        } else {
            Err("usage: mini-elf-gnuhash <input>...".to_owned())
        };
    }
...
fn format_gnu_hash(header: Elf64Header, file: &[u8]) -> Result<String, String> {
    ...
    let Some(hash_address) = unique_tag_value(&entries, DT_GNU_HASH, "DT_GNU_HASH")? else {
        return Ok("No DT_GNU_HASH entry found.\n".to_owned());
    };
    ...
    let mut dynamic_symbol_count = symbol_offset;
    for index in 0..bucket_count {
        ...
        if symbol != 0 {
            ...
            dynamic_symbol_count = dynamic_symbol_count.max(end);
        }
        buckets.push(symbol);
    }

    let mut output = format!(
        "GNU DT_GNU_HASH at {hash_address:#x}: {bucket_count} buckets, symbol offset {symbol_offset}, {bloom_count} bloom words, bloom shift {bloom_shift}, dynamic symbols through {dynamic_symbol_count}\n"
    );
