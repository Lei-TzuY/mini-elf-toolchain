...
fn run<I: Iterator<Item = OsString>>(args: I) -> Result<String, String> {
    ...
    if inputs.is_empty() {
        return Err("usage: mini-elf-dynrela-size32 <input>...".into());
    }
...
fn inspect(bytes: &[u8]) -> Result<String, String> {
    ...
    let mut out = format!(
        "Validated R_X86_64_SIZE32 relocations: entries={} symbols={symbol_count}\n",
        relasz / 24
    );
    ...
    for (index, entry) in rela_bytes.chunks_exact(24).enumerate() {
        ...
        if info as u32 != R_X86_64_SIZE32 {
            continue;
        }
        let symbol_index = info >> 32;
        if symbol_index == 0 || symbol_index >= symbol_count {
            return Err(format!(
                "R_X86_64_SIZE32 relocation {index} has invalid symbol index {symbol_index}"
            ));
        }
        map_memory(&phdrs, offset, 4, PF_W, "SIZE32 target")?;
        ...
        if section_index == 0 {
            ...
            if section_index == SHN_ABS {
                return Err(format!("R_X86_64_SIZE32 relocation {index} references an absolute symbol; absolute-symbol semantics are outside this bounded slice"));
            }
            if symbol_info & 0x0f == STT_TLS {
                return Err(format!("R_X86_64_SIZE32 relocation {index} references TLS; TLS semantics are outside this bounded slice"));
            }
            let value = i128::from(symbol_size) + i128::from(addend);
            let value = u32::try_from(value).map_err(|_| {
                format!("R_X86_64_SIZE32 relocation {index} result does not fit u32")
            })?;
    ...
    if found == 0 {
        return Err("DT_RELA contains no R_X86_64_SIZE32 relocations".into());
    }
    Ok(out)
}
...
fn dynamic_symbol_count(
    ...
    let address = gnu_hash.ok_or_else(|| {
        "R_X86_64_SIZE32 validation requires DT_HASH or DT_GNU_HASH to bound DT_SYMTAB".to_owned()
    })?;
