    ...
    I: Iterator<Item = OsString>,
{
    ...
    if args.is_empty() || args[0] == "--help" || args[0] == "-h" {
        return if args.len() <= 1 {
            Ok("usage: mini-elf-dynrel-relative --load-bias <address> <input>...\n".to_owned())
        } else {
            Err("usage: mini-elf-dynrel-relative --load-bias <address> <input>...".to_owned())
        };
    }
    ...
    if inputs.is_empty() {
        return Err("usage: mini-elf-dynrel-relative --load-bias <address> <input>...".to_owned());
    }
...
fn format_relative_relocations(
    ...
    let rel = unique_tag_value(&entries, DT_REL, "DT_REL")?;
    let rel_size = unique_tag_value(&entries, DT_RELSZ, "DT_RELSZ")?;
    let rel_entry_size = unique_tag_value(&entries, DT_RELENT, "DT_RELENT")?;

    let present = usize::from(rel.is_some())
        + usize::from(rel_size.is_some())
        + usize::from(rel_entry_size.is_some());
    if present == 0 {
    ...
    let rel_size = rel_size.unwrap();
    let rel_entry_size = rel_entry_size.unwrap();
    if rel_entry_size != ELF64_REL_SIZE {
        return Err(format!(
            "DT_RELENT is {rel_entry_size}, expected {ELF64_REL_SIZE} for ELF64"
        ));
    }
    if rel_size % rel_entry_size != 0 {
        return Err(format!(
            "DT_RELSZ {rel_size} is not a multiple of DT_RELENT {rel_entry_size}"
        ));
    }
    ...
    let count = rel_size / rel_entry_size;
    let mut relatives = Vec::new();
    for index in 0..count {
        let relative = index
            .checked_mul(rel_entry_size)
            .ok_or_else(|| "DT_REL entry offset overflows u64".to_owned())?;
        let offset = table_offset
            .checked_add(relative)
            .ok_or_else(|| "DT_REL entry offset overflows u64".to_owned())?;
        let offset = usize::try_from(offset)
            .map_err(|_| "DT_REL entry offset does not fit usize".to_owned())?;
        ...
        if symbol != 0 {
            return Err(format!(
                "DT_REL entry {index} R_X86_64_RELATIVE uses non-zero symbol index {symbol}"
            ));
        }
        let target_offset = map_virtual_file_range(
            ...
            relocation_offset,
            8,
            &format!("DT_REL entry {index} relocation target"),
        )?;
        let target_offset = usize::try_from(target_offset)
            .map_err(|_| "DT_REL relocation target offset does not fit usize".to_owned())?;
        let addend = read_i64(file, target_offset);
        let value = add_signed(load_bias, addend).ok_or_else(|| {
            format!(
                "DT_REL entry {index} R_X86_64_RELATIVE value overflows u64: load bias {load_bias:#x} + implicit addend {addend}"
            )
        })?;
    ...
    let mut output = format!(
        "DT_REL contains {count} entries, {} R_X86_64_RELATIVE relocations:\n",
        relatives.len()
    );
    output.push_str(&format!("Load bias: {load_bias:#018x}\n"));
    output.push_str("  Offset             Implicit addend        Value\n");
