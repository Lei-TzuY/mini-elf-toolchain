    ...
    I: Iterator<Item = OsString>,
{
    ...
    {
        return Err("usage: mini-elf-lsda-relative <input>...".to_owned());
    }
...
fn inspect(file: &[u8]) -> Result<String, String> {
    ...
    if read_u16(file, 16) != ET_DYN {
        return Err("mini-elf-lsda-relative requires an ET_DYN image".to_owned());
    }
    ...
    let (lsda_index, lsda_header) =
        lsda.ok_or_else(|| "missing .gcc_except_table section".to_owned())?;
    if lsda_header.flags & SHF_ALLOC == 0 {
        return Err(format!(
            ".gcc_except_table section {lsda_index} is not SHF_ALLOC"
        ));
    }
    let lsda_bytes = section_bytes(file, lsda_header, ".gcc_except_table")?;
    let (_, rela_header) = rela_dyn.ok_or_else(|| "missing .rela.dyn section".to_owned())?;
    if rela_header.section_type != SHT_RELA {
        return Err(format!(
            ".rela.dyn has section type {}, expected SHT_RELA",
            rela_header.section_type
        ));
    }
    ...
    if rela_header.size % rela_header.entsize != 0 {
        return Err(".rela.dyn size is not a whole number of RELA entries".to_owned());
    }
    let rela_bytes = section_bytes(file, rela_header, ".rela.dyn")?;

    let (ttype_base, table_end, records) = parse_lsda(lsda_bytes)?;
    let max_filter = records
        ...
        .unwrap_or(0);
    if max_filter <= 0 {
        return Err("LSDA contains no positive type-table filters".to_owned());
    }
    let max_filter_usize = usize::try_from(max_filter)
        .map_err(|_| "positive LSDA type filter does not fit usize".to_owned())?;
    let type_table_start = ttype_base
        .checked_sub(
            max_filter_usize
                .checked_mul(TYPE_ENTRY_SIZE)
                .ok_or_else(|| "LSDA type-table index byte count overflows usize".to_owned())?,
        )
        .ok_or_else(|| "LSDA type-table indices underflow .gcc_except_table".to_owned())?;
    if type_table_start < table_end {
        return Err("LSDA type-table entries overlap the call-site table".to_owned());
    }
    if records.iter().any(|record| record.end > type_table_start) {
        return Err("LSDA action records overlap type-table entries".to_owned());
    }

    let mut output = format!(
        "Validated ET_DYN GNU LSDA relative type slots: section={} address={:#018x} ttype-base={:#x} max-type-index={}\n",
        lsda_index, lsda_header.addr, ttype_base, max_filter
    );

    for type_index in 1..=max_filter_usize {
        if !records
            .iter()
            .any(|record| record.type_filter == type_index as i64)
        {
            continue;
        }
        ...
        if entry_end > lsda_bytes.len() {
            return Err(format!(
                "LSDA type-table index {type_index} exceeds .gcc_except_table"
            ));
        }
        let raw = read_i32(lsda_bytes, entry_start);
        let entry_address = lsda_header
            .addr
            .checked_add(
        ...
        map_file_backed_range(
            ...
            target,
            1,
            &format!("LSDA type-table index {type_index} relative target"),
        )?;
        output.push_str(&format!(
            "  type-index={type_index} entry=[{entry_start:#x},{entry_end:#x}) raw-sdata4={raw} slot={slot_address:#018x} rela-index={} target=B+{target:#018x}\n",
            relocation.0
        ));
    }
...
fn parse_lsda(bytes: &[u8]) -> Result<(usize, usize, Vec<ActionRecord>), String> {
    if bytes.len() < 6 {
    ...
    if ttype_base > bytes.len() {
        return Err("LSDA type-table base exceeds .gcc_except_table".to_owned());
    }
    ...
    if table_end > ttype_base || table_end > bytes.len() {
        return Err("LSDA call-site table crosses the type-table base".to_owned());
    }
    ...
    Ok((ttype_base, table_end, records))
}
...
fn parse_action_chain(
    ...
    loop {
        if seen.contains(&cursor) {
            return Err(format!(
                "LSDA call-site entry {entry_index} action chain contains a cycle"
            ));
        }
        seen.push(cursor);
        ...
        records.push(ActionRecord { end, type_filter });
        if next_displacement == 0 {
            break;
        }
        cursor = checked_add_signed(after_filter, next_displacement).ok_or_else(|| {
            format!("LSDA action record {one_based_offset} next displacement overflows")
        })?;
...
fn find_relative_relocation(
    bytes: &[u8],
    slot_address: u64,
    type_index: usize,
) -> Result<(usize, i64), String> {
    let mut found = None;
    for (index, entry) in bytes.chunks_exact(ELF64_RELA_SIZE).enumerate() {
        let offset = read_u64(entry, 0);
        if offset != slot_address {
            continue;
        }
        if found.is_some() {
            return Err(format!(
                "LSDA type-table index {type_index} pointer slot has multiple .rela.dyn relocations"
            ));
        }
        ...
        if relocation_type != R_X86_64_RELATIVE {
            return Err(format!(
                "LSDA type-table index {type_index} pointer slot relocation type {relocation_type} is unsupported; expected R_X86_64_RELATIVE"
            ));
        }
        if symbol != 0 {
            return Err(format!(
                "LSDA type-table index {type_index} R_X86_64_RELATIVE relocation has nonzero symbol index {symbol}"
            ));
        }
        found = Some((index, read_i64(entry, 16)));
    }
    found.ok_or_else(|| {
        format!(
            "LSDA type-table index {type_index} pointer slot has no matching .rela.dyn relocation"
        )
    })
}
...
fn checked_add_signed_u64(base: u64, displacement: i64) -> Option<u64> {
    let next = i128::from(base).checked_add(i128::from(displacement))?;
...
fn section_headers(
    ...
    for index in 0..shnum {
        ...
        headers.push(SectionHeader {
            ...
            section_type: read_u32(file, offset + 4),
