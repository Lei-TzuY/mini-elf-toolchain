...
#[allow(dead_code)]
mod checked_lookup {
    include!("mini-elf-sysv-hash-lookup.rs");
    ...
    for input in &args[1..] {
        ...
        let body = if let Some(index) = index {
            ...
            if eligibility.eligible {
                format!(
                    "SysV external lookup: symbol={symbol} index={index} bind={} visibility={} shndx={:#06x}\n",
                    eligibility.bind, eligibility.visibility, eligibility.shndx
                )
            } else {
                format!("SysV external lookup: symbol={symbol} not-found\n")
            }
        } else {
            format!("SysV external lookup: symbol={symbol} not-found\n")
        };
...
fn usage() -> String {
    "usage: mini-elf-sysv-hash-external-lookup <symbol> <input>...".to_owned()
}
...
fn parse_lookup_index(output: &str) -> Result<Option<u32>, String> {
    ...
    let start = output
        .find(marker)
        .ok_or_else(|| "checked SysV hash lookup returned an unrecognized result".to_owned())?
        + marker.len();
    let end = output[start..]
        .find(' ')
        .map(|offset| start + offset)
        .ok_or_else(|| "checked SysV hash lookup omitted the symbol index terminator".to_owned())?;
    output[start..end]
        .parse::<u32>()
        .map(Some)
        .map_err(|_| "checked SysV hash lookup returned an invalid symbol index".to_owned())
}
