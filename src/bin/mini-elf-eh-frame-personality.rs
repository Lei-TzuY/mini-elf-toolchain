    ...
    I: Iterator<Item = OsString>,
{
    ...
    {
        return Err("usage: mini-elf-eh-frame-personality <input>...".to_owned());
    }
...
fn inspect(file: &[u8]) -> Result<String, String> {
    ...
    let mut personalities = Vec::with_capacity(
    ...
    for index in 0..count {
        ...
        personalities.push(validate_fde_personality(file, &headers, fde, index)?);
    }

    let mut output = format!("Validated GNU zPR personalities: {}\n", personalities.len());
    for (index, personality) in personalities.iter().enumerate() {
        output.push_str(&format!(
            "FDE {index}: personality={:#018x} executable_load={}\n",
            personality.address, personality.load_index
        ));
    }
    Ok(output)
}

fn validate_fde_personality(
    file: &[u8],
    ...
    if fde_length < 8 || fde_length == u32::MAX {
        return Err(format!(
    ...
    validate_zpr_cie(file, headers, cie, index)
}
...
fn validate_zpr_cie(
    ...
    if &file[aug_start..aug_end] != b"zPR" {
        return Err(format!("FDE {index} CIE augmentation is not supported zPR"));
    }
    ...
    cursor = next;
    if augmentation_length != 6 {
        return Err(format!(
            "FDE {index} CIE zPR payload has unsupported length {augmentation_length}"
        ));
    }
    ...
    if payload_end > cie_end {
        return Err(format!("FDE {index} CIE zPR payload is truncated"));
    }
    ...
    let field_delta = personality_field_offset
    ...
    let load_index = executable_file_backed_range(headers, personality, personality_end).ok_or_else(|| {
        format!(
