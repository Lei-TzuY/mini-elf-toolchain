    ...
    I: Iterator<Item = OsString>,
{
    ...
    if args.is_empty() || args[0] == "--help" || args[0] == "-h" {
        return if args.len() <= 1 {
            Ok("usage: mini-elf-dynrel <input>...\n".to_owned())
        } else {
            Err("usage: mini-elf-dynrel <input>...".to_owned())
        };
    }
...
fn format_dynamic_relocations(header: Elf64Header, file: &[u8]) -> Result<String, String> {
    ...
    let mut output = format!("DT_REL contains {count} entries:\n");
    output.push_str("  Offset             Info               Type                 Sym Name\n");
    for index in 0..count {
        ...
        output.push_str(&format!(
            "  {relocation_offset:#018x} {info:#018x} {:<20} {symbol:>4} {symbol_name}\n",
...
fn dynamic_symbols(
    ...
    if present != 4 {
        return Err(
            "DT_REL symbol resolution requires DT_SYMTAB, DT_SYMENT, DT_STRTAB, and DT_STRSZ together"
                .to_owned(),
        );
    }
...
fn dynamic_symbol_count(
    ...
    let Some(hash_address) = unique_tag_value(entries, DT_GNU_HASH, "DT_GNU_HASH")? else {
        return Err(
            "DT_REL symbol resolution requires DT_HASH or DT_GNU_HASH to bound DT_SYMTAB"
                .to_owned(),
        );
    };
...
fn dynamic_symbol_name(
    ...
    if symbol_index >= symbols.symbol_count {
        return Err(format!(
            "DT_REL symbol index {symbol_index} is outside dynamic symbol count {}",
