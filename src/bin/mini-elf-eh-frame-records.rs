    ...
    I: Iterator<Item = OsString>,
{
    ...
    if args.is_empty() || args[0] == "--help" || args[0] == "-h" {
        return if args.len() <= 1 {
            Ok("usage: mini-elf-eh-frame-records <input>...\n".to_owned())
        } else {
...
fn format_records(header: Elf64Header, file: &[u8]) -> Result<String, String> {
    ...
    for entry_index in 0..fde_count {
        ...
        records.push(validate_fde_record(
            file,
            &program_headers,
            fde_address,
            entry_index,
    ...
    let mut output = String::new();
    output.push_str(&format!("Validated FDE records: {}\n", records.len()));
    for (index, record) in records.iter().enumerate() {
        output.push_str(&format!(
            "FDE {index}: address={:#018x} size={} cie={:#018x} cie_size={}\n",
            record.address, record.total_size, record.cie_address, record.cie_total_size
        ));
    }
    Ok(output)
}

fn validate_fde_record(
    file: &[u8],
    program_headers: &[ProgramHeader],
    fde_address: u64,
    entry_index: u64,
) -> Result<FdeRecord, String> {
    ...
    if cie.total_size < 9 {
        return Err(format!(
            "FDE {entry_index} CIE record at {cie_address:#x} is too short to contain the CIE id and version"
        ));
    }

    Ok(FdeRecord {
        address: fde.address,
        total_size: fde.total_size,
        cie_address,
        cie_total_size: cie.total_size,
    })
}
