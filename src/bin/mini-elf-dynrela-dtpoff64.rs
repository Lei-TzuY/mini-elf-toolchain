...
fn usage() -> String {
    "usage: mini-elf-dynrela-dtpoff64 --load-bias <address> <input>...".into()
}
...
fn inspect(file: &[u8], bias: u64) -> Result<String, String> {
    ...
    if file.len() < 64 || u16at(file, 16) != ET_DYN {
        return Err("mini-elf-dynrela-dtpoff64 requires an ET_DYN image".into());
    }
    ...
    for (idx, e) in relas.chunks_exact(RELAENT).enumerate() {
        let off = u64at(e, 0);
        let info = u64at(e, 8);
        if info as u32 != R_X86_64_DTPOFF64 {
            continue;
        }
        let si = info >> 32;
        if si == 0 || si >= count {
            return Err(format!("R_X86_64_DTPOFF64 relocation {idx} references invalid dynamic symbol index {si} (count {count})"));
        }
        memory_range(
            &ph,
            off,
            8,
            PF_W,
            &format!("R_X86_64_DTPOFF64 relocation {idx} target"),
        )?;
        ...
        let result = u64::try_from(wide).map_err(|_| {
            format!("R_X86_64_DTPOFF64 relocation {idx} result does not fit unsigned 64-bit")
        })?;
        let rt = bias.checked_add(off).ok_or_else(|| {
            format!("R_X86_64_DTPOFF64 relocation {idx} runtime target overflows u64")
        })?;
    ...
    if found == 0 {
        return Err("DT_RELA contains no R_X86_64_DTPOFF64 relocations".into());
    }
    Ok(out)
}
