...
fn run<I: Iterator<Item = OsString>>(args: I) -> Result<String, String> {
    ...
    while i < args.len() {
        ...
        if s == "--load-bias" {
            ...
            if tls_block_offset
                .replace(parse_i64(&args[i].to_string_lossy(), "TLS block offset")?)
                .is_some()
            {
            ...
            if tls_block_offset
                .replace(parse_i64(v, "TLS block offset")?)
                .is_some()
            {
                return Err("duplicate --tls-block-offset option".into());
            }
...
fn inspect(file: &[u8], bias: u64, tls_block_offset: i64) -> Result<String, String> {
    ...
    if file.len() < 64 || u16at(file, 16) != ET_DYN {
        return Err("mini-elf-dynrela-tpoff32 requires an ET_DYN image".into());
    }
    ...
    for (idx, e) in relas.chunks_exact(RELAENT).enumerate() {
        let off = u64at(e, 0);
        let info = u64at(e, 8);
        if info as u32 != R_X86_64_TPOFF32 {
            continue;
        }
        let si = info >> 32;
        if si == 0 || si >= count {
            return Err(format!("R_X86_64_TPOFF32 relocation {idx} references invalid dynamic symbol index {si} (count {count})"));
        }
        memory_range(
            &ph,
            off,
            4,
            PF_W,
            &format!("R_X86_64_TPOFF32 relocation {idx} target"),
        )?;
        ...
        let value = u64at(s, 8);
        if sh == SHN_UNDEF {
            return Err(format!("R_X86_64_TPOFF32 relocation {idx} references undefined TLS symbol; dependency lookup is outside this bounded slice"));
        }
        if sh == SHN_ABS || typ != STT_TLS {
            return Err(format!(
                "R_X86_64_TPOFF32 relocation {idx} requires a defined STT_TLS symbol"
            ));
        }
        let add = i64at(e, 16);
        let wide = tls_block_offset as i128 + value as i128 + add as i128;
        let result = i32::try_from(wide).map_err(|_| {
            format!("R_X86_64_TPOFF32 relocation {idx} result does not fit signed 32-bit")
        })?;
        let rt = bias.checked_add(off).ok_or_else(|| {
            format!("R_X86_64_TPOFF32 relocation {idx} runtime target overflows u64")
        })?;
    ...
    if found == 0 {
        return Err("DT_RELA contains no R_X86_64_TPOFF32 relocations".into());
    }
    Ok(out)
}
