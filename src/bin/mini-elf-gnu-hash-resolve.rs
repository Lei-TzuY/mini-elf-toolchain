...
mod checked_external_lookup {
    ...
    pub fn lookup(symbol: &str, input: &std::ffi::OsStr) -> Result<Option<u32>, String> {
        ...
        let end = output[start..]
            ...
            .ok_or_else(|| {
                "checked GNU external lookup omitted the symbol index terminator".to_owned()
            })?;
        output[start..end]
            .parse::<u32>()
            .map(Some)
            .map_err(|_| "checked GNU external lookup returned an invalid symbol index".to_owned())
    }
}
...
fn run<I: Iterator<Item = OsString>>(args: I) -> Result<String, String> {
    ...
    Ok(format!("GNU external resolve: symbol={symbol} not-found\n"))
}

fn render_found(symbol: &str, input: &OsStr, index: u32) -> String {
    format!(
        "GNU external resolve: symbol={symbol} file={} index={index}\n",
        input.to_string_lossy()
    )
}

fn usage() -> String {
    "usage: mini-elf-gnu-hash-resolve <symbol> <input>...".to_owned()
}
