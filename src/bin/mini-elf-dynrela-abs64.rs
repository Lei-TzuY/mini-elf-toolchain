...
fn usage() -> String {
    "usage: mini-elf-dynrela-abs64 --load-bias <address> <input>...".to_owned()
}
...
fn inspect(file: &[u8], load_bias: u64) -> Result<String, String> {
    ...
    if read_u16(file, 16) != ET_DYN {
        return Err("mini-elf-dynrela-abs64 requires an ET_DYN image".to_owned());
    }
    ...
    let mut output = format!(
        "Validated R_X86_64_64 relocations: load-bias={load_bias:#018x} rela={:#018x} entries={} symbols={symbol_count}\n",
        metadata.rela,
    ...
    for (index, entry) in rela.chunks_exact(ELF64_RELA_SIZE).enumerate() {
        ...
        if symbol_index == 0 || symbol_index >= symbol_count {
            return Err(format!(
                "R_X86_64_64 relocation {index} references invalid dynamic symbol index {symbol_index} (count {symbol_count})"
            ));
        }
        map_memory_range(
            &headers,
            ...
            8,
            PF_W,
            &format!("R_X86_64_64 relocation {index} target"),
        )?;
        ...
        if section_index == SHN_UNDEF {
            return Err(format!(
                "R_X86_64_64 relocation {index} references undefined dynamic symbol index {symbol_index}; external lookup is outside this bounded slice"
            ));
        }
        if section_index == SHN_ABS {
            return Err(format!(
                "R_X86_64_64 relocation {index} references absolute dynamic symbol index {symbol_index}; absolute-symbol semantics are outside this bounded slice"
            ));
        }
        if symbol_info & 0x0f == STT_TLS {
            return Err(format!(
                "R_X86_64_64 relocation {index} references TLS dynamic symbol index {symbol_index}; TLS semantics are outside this bounded slice"
            ));
        }
        map_memory_range(
            &headers,
            ...
            1,
            0,
            &format!("R_X86_64_64 relocation {index} symbol value"),
        )?;
    ...
    if found == 0 {
        return Err("DT_RELA contains no R_X86_64_64 relocations".to_owned());
    }
    Ok(output)
}
...
fn dynamic_symbol_count(
    ...
    let address = gnu_hash.ok_or_else(|| {
        "R_X86_64_64 validation requires DT_HASH or DT_GNU_HASH to bound DT_SYMTAB".to_owned()
    })?;
