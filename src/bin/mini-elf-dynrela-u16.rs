...
fn usage() -> String {
    "usage: mini-elf-dynrela-u16 --load-bias <address> <input>...".into()
}
...
fn inspect(file: &[u8], bias: u64) -> Result<String, String> {
    ...
    if file.len() < 64 || read_u16(file, 16) != ET_DYN {
        return Err("mini-elf-dynrela-u16 requires an ELF64 ET_DYN image".into());
    }
    ...
    let mut out = format!(
        "Validated R_X86_64_16 relocations: load-bias={bias:#018x} rela={rela:#018x} entries={} symbols={symbol_count}\n",
        relasz / relaent
    );
    ...
    for (index, entry) in rela_bytes.chunks_exact(RELA_SIZE).enumerate() {
        ...
        if info as u32 != R_X86_64_16 {
            continue;
        }
        if sym_index == 0 || sym_index >= symbol_count {
            return Err(format!("R_X86_64_16 relocation {index} references invalid dynamic symbol index {sym_index} (count {symbol_count})"));
        }
        map_memory(
            &phdrs,
            offset,
            2,
            PF_W,
            &format!("R_X86_64_16 relocation {index} target"),
        )?;
        ...
        if shndx == SHN_UNDEF {
            return Err(format!("R_X86_64_16 relocation {index} references undefined dynamic symbol index {sym_index}; external lookup is outside this bounded slice"));
        }
        if shndx == SHN_ABS {
            return Err(format!("R_X86_64_16 relocation {index} references an absolute dynamic symbol; absolute-symbol semantics are outside this bounded slice"));
        }
        if info_byte & 0x0f == STT_TLS {
            return Err(format!("R_X86_64_16 relocation {index} references TLS; TLS semantics are outside this bounded slice"));
        }
        map_memory(
            &phdrs,
            value,
            1,
            0,
            &format!("R_X86_64_16 relocation {index} symbol value"),
        )?;
        let name = dyn_string(strtab_bytes, name_off, sym_index)?;
        let runtime_target = bias.checked_add(offset).ok_or_else(|| {
            format!("R_X86_64_16 relocation {index} runtime target overflows u64")
        })?;
        let runtime_symbol = bias.checked_add(value).ok_or_else(|| {
            format!("R_X86_64_16 relocation {index} runtime symbol overflows u64")
        })?;
        let addend = read_i64(entry, 16);
        let relocated = add_signed(runtime_symbol, addend)
            .ok_or_else(|| format!("R_X86_64_16 relocation {index} S + A overflows u64"))?;
        let value16 = u16::try_from(relocated).map_err(|_| {
            format!(
                "R_X86_64_16 relocation {index} result {relocated:#x} does not fit unsigned 16 bits"
            )
        })?;
        out.push_str(&format!("  index={index} symbol={sym_index}:{name} target=B+{offset:#018x}=>{runtime_target:#018x} symbol-value=B+{value:#018x}=>{runtime_symbol:#018x} addend={addend} result={value16}\n"));
        found += 1;
    }
    if found == 0 {
        return Err("DT_RELA contains no R_X86_64_16 relocations".into());
    }
    Ok(out)
}
...
fn dynamic_symbol_count(
    ...
    let address = gnu_hash.ok_or_else(|| {
        "R_X86_64_16 validation requires DT_HASH or DT_GNU_HASH to bound DT_SYMTAB".to_owned()
    })?;
