    ...
    I: Iterator<Item = OsString>,
{
    ...
    {
        return Err("usage: mini-elf-eh-frame-lsda <input>...".to_owned());
    }
...
fn inspect(file: &[u8]) -> Result<String, String> {
    ...
    let eh_vaddr_end = eh
        .vaddr
        .checked_add(eh.filesz)
        .ok_or_else(|| "PT_GNU_EH_FRAME virtual range overflows u64".to_owned())?;
    if file_backed_range(&headers, eh.vaddr, eh_vaddr_end).is_none() {
        return Err("PT_GNU_EH_FRAME is not fully contained in one file-backed PT_LOAD".to_owned());
    }
    ...
    for index in 0..count {
        ...
        entries.push(validate_fde_lsda(file, &headers, fde, index)?);
    }

    let mut output = format!("Validated GNU zPLR LSDAs: {}\n", entries.len());
    for (index, entry) in entries.iter().enumerate() {
        output.push_str(&format!(
            "FDE {index}: personality={:#018x} lsda={:#018x} load={}\n",
            entry.personality, entry.lsda, entry.load_index
        ));
    }
    Ok(output)
}

fn validate_fde_lsda(
    file: &[u8],
    headers: &[ProgramHeader],
    fde: u64,
    index: u64,
) -> Result<LsdaMetadata, String> {
    let (_, fde_offset) = map_file_backed(headers, fde)
    ...
    let fixed_end = fde_offset
        .checked_add(16)
        .ok_or_else(|| format!("FDE {index} fixed fields overflow usize"))?;
    if fixed_end > fde_end {
        return Err(format!("FDE {index} is truncated before augmentation data"));
    }
    ...
    let cie_metadata = validate_zplr_cie(file, headers, cie, index)?;

    let (augmentation_length, mut cursor) =
        read_uleb(file, fixed_end, fde_end, "FDE augmentation length")?;
    if augmentation_length != 4 {
        return Err(format!(
            "FDE {index} zPLR augmentation payload has unsupported length {augmentation_length}"
        ));
    }
    let payload_end = cursor
        .checked_add(4)
        .ok_or_else(|| format!("FDE {index} LSDA payload overflows usize"))?;
    if payload_end > fde_end {
        return Err(format!("FDE {index} LSDA payload is truncated"));
    }

    let field_delta = cursor
        .checked_sub(fde_offset)
        .ok_or_else(|| format!("FDE {index} LSDA field offset underflows"))?;
    let lsda_field = fde
        .checked_add(
            u64::try_from(field_delta)
                .map_err(|_| format!("FDE {index} LSDA field offset does not fit u64"))?,
        )
        .ok_or_else(|| format!("FDE {index} LSDA field address overflows u64"))?;
    let lsda = checked_add_i32(lsda_field, read_i32(file, cursor))
        .ok_or_else(|| format!("FDE {index} LSDA pointer arithmetic overflows u64"))?;
    cursor = payload_end;
    if cursor > fde_end {
        return Err(format!("FDE {index} LSDA payload exceeds record"));
    }
    let lsda_end = lsda
        .checked_add(1)
    ...
    Ok(LsdaMetadata {
        personality: cie_metadata.personality,
        lsda,
        load_index,
    })
}

fn validate_zplr_cie(
    file: &[u8],
    headers: &[ProgramHeader],
    cie: u64,
    index: u64,
) -> Result<CieMetadata, String> {
    let (_, cie_offset) = map_file_backed(headers, cie)
    ...
    if cie_offset
        .checked_add(9)
        .ok_or_else(|| format!("FDE {index} CIE fixed fields overflow usize"))?
        > cie_end
    ...
    cursor = next;
    if augmentation_length != 7 {
        return Err(format!(
            "FDE {index} CIE zPLR payload has unsupported length {augmentation_length}"
        ));
    }
    let payload_end = cursor
        .checked_add(7)
    ...
    if payload_end > cie_end {
        return Err(format!("FDE {index} CIE zPLR payload is truncated"));
    }
    if file[cursor] != DW_EH_PE_PCREL_SDATA4 {
        return Err(format!(
            "FDE {index} CIE personality encoding is not pcrel/sdata4"
        ));
    }

    let personality_field_offset = cursor + 1;
    let personality_delta = personality_field_offset
        .checked_sub(cie_offset)
        .ok_or_else(|| format!("FDE {index} CIE personality field offset underflows"))?;
    let personality_field = cie
        .checked_add(
            u64::try_from(personality_delta).map_err(|_| {
    ...
    let personality = checked_add_i32(personality_field, read_i32(file, personality_field_offset))
        .ok_or_else(|| format!("FDE {index} CIE personality pointer arithmetic overflows u64"))?;
    let personality_end = personality
        .checked_add(1)
        .ok_or_else(|| format!("FDE {index} CIE personality target overflows u64"))?;
    executable_file_backed_range(headers, personality, personality_end).ok_or_else(|| {
        format!(
            "FDE {index} CIE personality target {personality:#x} is not executable file-backed PT_LOAD data"
        )
    })?;
...
fn file_backed_range(headers: &[ProgramHeader], start: u64, end: u64) -> Option<usize> {
    headers.iter().enumerate().find_map(|(index, header)| {
...
fn map_file_backed(headers: &[ProgramHeader], address: u64) -> Option<(usize, usize)> {
    headers.iter().enumerate().find_map(|(index, header)| {
        if header.kind != PT_LOAD {
            return None;
        }
        ...
        Some((index, usize::try_from(offset).ok()?))
    })
}
...
fn record_end(
    ...
    let (start_load, start) = map_file_backed(headers, address)
        .ok_or_else(|| format!("{kind} {index} is not file-backed"))?;
    let last = end_address
        .checked_sub(1)
        .ok_or_else(|| format!("{kind} {index} range underflows"))?;
    let (end_load, _) = map_file_backed(headers, last)
        .ok_or_else(|| format!("{kind} {index} crosses a file-backed PT_LOAD boundary"))?;
...
fn program_headers(header: Elf64Header, file: &[u8]) -> Result<Vec<ProgramHeader>, String> {
    ...
    for index in 0..header.program_header_count {
        let relative = u64::from(index)
            ...
            .ok_or_else(|| "program-header relative offset overflows u64".to_owned())?;
        let offset = header
        ...
        let end = offset
            .checked_add(usize::from(ELF64_PROGRAM_HEADER_SIZE))
            .ok_or_else(|| "program-header file range overflows usize".to_owned())?;
        if end > file.len() {
            return Err(format!("program header {index} exceeds input"));
        }
