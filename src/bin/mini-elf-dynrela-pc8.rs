...
fn run<I: Iterator<Item = OsString>>(args: I) -> Result<String, String> {
    ...
    if inputs.is_empty() {
        return Err("usage: mini-elf-dynrela-pc8 <input>...".into());
    }
...
fn inspect(bytes: &[u8]) -> Result<String, String> {
    ...
    let mut out = format!(
        "Validated R_X86_64_PC8 relocations: entries={} symbols={symbol_count}\n",
        relasz / 24
    );
    ...
    for (index, entry) in rela_bytes.chunks_exact(24).enumerate() {
        ...
        if info as u32 != R_X86_64_PC8 {
            continue;
        }
        let symbol_index = info >> 32;
        if symbol_index == 0 || symbol_index >= symbol_count {
            return Err(format!(
                "R_X86_64_PC8 relocation {index} has invalid symbol index {symbol_index}"
            ));
        }
        map_memory(&phdrs, offset, 1, PF_W, "PC8 target")?;
        ...
        if section_index == 0 {
            out.push_str(&format!(
                "  index={index} symbol={symbol_index}:{name} binding=external target={offset:#x} addend={addend} formula=S+A-P\n"
            ));
        } else {
            if section_index == SHN_ABS {
                return Err(format!(
                    "R_X86_64_PC8 relocation {index} references an absolute symbol; absolute-symbol semantics are outside this bounded slice"
                ));
            }
            if symbol_info & 0x0f == STT_TLS {
                return Err(format!(
                    "R_X86_64_PC8 relocation {index} references TLS; TLS semantics are outside this bounded slice"
                ));
            }
            map_memory(&phdrs, symbol_value, 1, 0, "PC8 symbol")?;
            let value = i128::from(symbol_value) + i128::from(addend) - i128::from(offset);
            i8::try_from(value)
                .map_err(|_| format!("R_X86_64_PC8 relocation {index} result does not fit i8"))?;
            out.push_str(&format!(
    ...
    if found == 0 {
        return Err("DT_RELA contains no R_X86_64_PC8 relocations".into());
    }
    Ok(out)
}
...
fn dynamic_symbol_count(
    ...
    if let Some(address) = hash {
        let header = map_file(bytes, phdrs, address, 8, 0, "DT_HASH header")?;
        return Ok(u64::from(read_u32(header, 4)));
    }
    let address = gnu_hash.ok_or_else(|| {
        "R_X86_64_PC8 validation requires DT_HASH or DT_GNU_HASH to bound DT_SYMTAB".to_owned()
    })?;
...
fn map_file<'a>(
    ...
    for header in headers
        .iter()
        .filter(|header| header.kind == PT_LOAD && header.flags & flags == flags)
    {
        let load_end = header
            .va
            .checked_add(header.filesz)
            .ok_or_else(|| format!("{label} segment range overflow"))?;
        if address >= header.va && end <= load_end {
