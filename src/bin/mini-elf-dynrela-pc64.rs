...
fn run<I: Iterator<Item = OsString>>(args: I) -> Result<String, String> {
    ...
    if inputs.is_empty() { return Err("usage: mini-elf-dynrela-pc64 <input>...".into()); }
    ...
    for (index, (name, body)) in rendered.into_iter().enumerate() {
        if index != 0 { out.push('\n'); }
        if multiple { out.push_str(&format!("File: {name}\n")); }
        out.push_str(&body);
    }
    Ok(out)
}
...
fn inspect(bytes: &[u8]) -> Result<String, String> {
    ...
    if dynamic_headers.len() != 1 { return Err("requires exactly one PT_DYNAMIC".into()); }
    let dynamic = file_range(bytes, dynamic_headers[0].off, dynamic_headers[0].filesz, "PT_DYNAMIC")?;
    if dynamic.len() % 16 != 0 { return Err("PT_DYNAMIC size is not a multiple of 16".into()); }

    let mut hash = None; let mut symtab = None; let mut syment = None; let mut strtab = None;
    let mut strsz = None; let mut rela = None; let mut relasz = None; let mut relaent = None;
    let mut terminated = false;
    for (index, entry) in dynamic.chunks_exact(16).enumerate() {
        let tag = read_i64(entry, 0); let value = read_u64(entry, 8);
        if tag == DT_NULL { terminated = true; break; }
        let slot = match tag {
            DT_HASH => &mut hash, DT_SYMTAB => &mut symtab, DT_SYMENT => &mut syment,
            DT_STRTAB => &mut strtab, DT_STRSZ => &mut strsz, DT_RELA => &mut rela,
            DT_RELASZ => &mut relasz, DT_RELAENT => &mut relaent, _ => continue,
        };
        if slot.replace(value).is_some() { return Err(format!("duplicate dynamic tag at entry {index}")); }
    }
    if !terminated { return Err("PT_DYNAMIC lacks DT_NULL".into()); }
    let hash = required(hash, "DT_HASH")?; let symtab = required(symtab, "DT_SYMTAB")?;
    let syment = required(syment, "DT_SYMENT")?; let strtab = required(strtab, "DT_STRTAB")?;
    let strsz = required(strsz, "DT_STRSZ")?; let rela = required(rela, "DT_RELA")?;
    let relasz = required(relasz, "DT_RELASZ")?; let relaent = required(relaent, "DT_RELAENT")?;
    if syment != 24 || relaent != 24 || relasz % 24 != 0 { return Err("unsupported dynamic table entry size".into()); }

    let hash_bytes = map_file(bytes, &phdrs, hash, 8, 0, "DT_HASH")?;
    let symbol_count = u64::from(read_u32(hash_bytes, 4));
    if symbol_count == 0 { return Err("DT_HASH reports zero symbols".into()); }
    ...
    let mut out = format!("Validated R_X86_64_PC64 relocations: entries={} symbols={symbol_count}\n", relasz / 24);
    let mut found = 0usize;
    for (index, entry) in rela_bytes.chunks_exact(24).enumerate() {
        let offset = read_u64(entry, 0); let info = read_u64(entry, 8);
        if info as u32 != R_X86_64_PC64 { continue; }
        let symbol_index = info >> 32;
        if symbol_index == 0 || symbol_index >= symbol_count {
            return Err(format!("R_X86_64_PC64 relocation {index} has invalid symbol index {symbol_index}"));
        }
        map_memory(&phdrs, offset, 8, PF_W, "PC64 target")?;
        let symbol_offset = usize::try_from(symbol_index.checked_mul(24).ok_or("symbol offset overflow")?).map_err(|_| "symbol offset too large")?;
        let symbol = symbol_bytes.get(symbol_offset..symbol_offset + 24).ok_or("dynamic symbol exceeds DT_SYMTAB")?;
        let name_offset = read_u32(symbol, 0) as usize; let symbol_info = symbol[4];
        let section_index = read_u16(symbol, 6); let symbol_value = read_u64(symbol, 8);
        if name_offset >= string_bytes.len() { return Err(format!("dynamic symbol {symbol_index} name offset is outside DT_STRTAB")); }
        ...
        if section_index == 0 {
            ...
            if section_index == SHN_ABS { return Err(format!("R_X86_64_PC64 relocation {index} references an absolute symbol; absolute-symbol semantics are outside this bounded slice")); }
            if symbol_info & 0x0f == STT_TLS { return Err(format!("R_X86_64_PC64 relocation {index} references TLS; TLS semantics are outside this bounded slice")); }
            map_memory(&phdrs, symbol_value, 1, 0, "PC64 symbol")?;
            let value = i128::from(symbol_value) + i128::from(addend) - i128::from(offset);
            i64::try_from(value).map_err(|_| format!("R_X86_64_PC64 relocation {index} result does not fit i64"))?;
            out.push_str(&format!("  index={index} symbol={symbol_index}:{name} binding=same-image target={offset:#x} addend={addend} result={value}\n"));
        }
        found += 1;
    }
    if found == 0 { return Err("DT_RELA contains no R_X86_64_PC64 relocations".into()); }
    Ok(out)
}

fn program_headers(bytes: &[u8]) -> Result<Vec<Phdr>, String> {
    let offset = usize::try_from(read_u64(bytes, 32)).map_err(|_| "phoff too large")?;
    let entry_size = usize::from(read_u16(bytes, 54)); let count = usize::from(read_u16(bytes, 56));
    if entry_size != 56 { return Err("unsupported program-header size".into()); }
    ...
    if end > bytes.len() { return Err("program-header table exceeds input".into()); }
    let mut headers = Vec::with_capacity(count);
    for index in 0..count {
        let start = offset + index * entry_size;
        let header = Phdr { kind: read_u32(bytes, start), flags: read_u32(bytes, start + 4), off: read_u64(bytes, start + 8), va: read_u64(bytes, start + 16), filesz: read_u64(bytes, start + 32), memsz: read_u64(bytes, start + 40) };
        if header.filesz > header.memsz { return Err(format!("program header {index} filesz exceeds memsz")); }
...
fn map_memory(headers: &[Phdr], address: u64, size: u64, flags: u32, label: &str) -> Result<(), String> {
    ...
    for h in headers.iter().filter(|h| h.kind == PT_LOAD && h.flags & flags == flags) {
        ...
        if address >= h.va && end <= load_end { return Ok(()); }
    }
    Err(format!("{label} is not contained in matching PT_LOAD"))
}

fn required(value: Option<u64>, name: &str) -> Result<u64, String> { value.ok_or_else(|| format!("missing {name}")) }
