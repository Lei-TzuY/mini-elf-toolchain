...
fn make_relative64(shared: &Path, output: &Path) -> Vec<u8> {
    ...
    assert_eq!(
        symbol, 0,
        "fixture should begin with a symbol-free relative relocation"
    );
...
fn relative64_rejects_non_writable_target() {
    ...
    let bad = dir.join("bad.so");
    let mut bytes = make_relative64(&shared, &bad);
    let target = program_headers(&bytes)
        .into_iter()
        .find(|h| h.0 == 1 && h.1 & 2 == 0 && h.5 >= 8)
        .expect("fixture should have a non-writable PT_LOAD")
        .3;
    let rela = rela_offset(&bytes);
