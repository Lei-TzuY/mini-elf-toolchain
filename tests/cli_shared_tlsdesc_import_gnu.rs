...
fn temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-shared-tlsdesc-import-{label}-{}-{nonce}",
        std::process::id()
    ));
...
#[test]
fn shared_tlsdesc_import_binds_provider_tls_and_preserves_thread_instances() {
    if !have_tools() {
        return;
    }
...
"#,
    ...
    let consumer_object = assemble(
        ...
        "consumer",
        r#".section .text
.globl read_provider_tlsdesc
.type read_provider_tlsdesc,@function
.extern provider_tls
.type provider_tls,@tls_object
read_provider_tlsdesc:
    leaq provider_tls@TLSDESC(%rip), %rax
    call *provider_tls@TLSCALL(%rax)
    mov %fs:(%rax), %rax
    ret
.size read_provider_tlsdesc, .-read_provider_tlsdesc

.globl write_provider_tlsdesc
.type write_provider_tlsdesc,@function
write_provider_tlsdesc:
    push %rbx
    mov %rdi, %rbx
    leaq provider_tls@TLSDESC(%rip), %rax
    call *provider_tls@TLSCALL(%rax)
    ...
    pop %rbx
    ret
.size write_provider_tlsdesc, .-write_provider_tlsdesc
"#,
    );
    ...
    assert!(input_relocations.contains("provider_tls"));
    ...
    assert!(
        ...
        "cross-DSO TLSDESC must remain descriptor-based: {relocations}"
    );
...
int main(int argc, char **argv) {
    ...
    read_fn read_tls = (read_fn)dlsym(handle, "read_provider_tlsdesc");
    write_fn write_tls = (write_fn)dlsym(handle, "write_provider_tlsdesc");
    if (!read_tls || !write_tls) return 122;

    if (read_tls() != UINT64_C(0x123456789abcdef0)) return 123;
    write_tls(UINT64_C(0xaaaaaaaaaaaaaaaa));
    if (read_tls() != UINT64_C(0xaaaaaaaaaaaaaaaa)) return 124;

    struct ctx ctx = { read_tls, write_tls, -1 };
    pthread_t thread;
    if (pthread_create(&thread, 0, worker, &ctx) != 0) return 125;
    if (pthread_join(thread, 0) != 0) return 126;
    if (ctx.result != 0) return ctx.result;
    if (read_tls() != UINT64_C(0xaaaaaaaaaaaaaaaa)) return 127;

    return dlclose(handle) == 0 ? 0 : 128;
}
"#,
        )
        .unwrap();
...
#[test]
fn shared_tlsdesc_provider_matching_requires_tls_symbol_type() {
    if !have_tools() {
        return;
    }
