...
fn assemble(dir: &Path, stem: &str, src: &str) -> PathBuf {
    let s = dir.join(format!("{stem}.s"));
    let o = dir.join(format!("{stem}.o"));
...
fn dynamic_pie_binds_gotpcrel64_through_loader_got() {
    ...
    let Some(interp) = dynamic_linker() else {
        return;
    };
    let dir = temp_dir();
    let provider_o = assemble(
        &dir,
        "provider",
        r#".data
...
"#,
    ...
    assert!(
        input_relocs.contains("R_X86_64_GOTPCREL64"),
        "{input_relocs}"
    );

    let mini = dir.join("mini");
    let r = Command::new(env!("CARGO_BIN_EXE_mini-elf-toolchain"))
        .args(["link", "-o"])
        .arg(&mini)
        .arg("--dynamic-pie")
        .arg("--dynamic-linker")
    ...
    assert!(
        relocs
            .lines()
            .any(|l| l.contains("R_X86_64_GLOB_DAT") && l.contains("provider_value")),
        "{relocs}"
    );
