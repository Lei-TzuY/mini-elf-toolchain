...
fn run(inputs: &[&Path]) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-pc8"));
    for input in inputs {
        command.arg(input);
    }
...
fn rela_offset(bytes: &[u8]) -> usize {
    ...
    loop {
        if read_u64(bytes, offset + 8) as u32 == R_X86_64_PC8 {
            return offset;
        }
        offset += 24;
    }
...
#[test]
fn validates_gnu_pc8_external_relocation() {
    let dir = temp("good");
    ...
    assert!(
        String::from_utf8_lossy(&readelf.stdout).contains("R_X86_64_PC8"),
        "{}",
    ...
    assert!(stdout.contains("Validated R_X86_64_PC8 relocations"));
...
fn rejects_invalid_symbol_index() {
    ...
    bytes[rela + 8..rela + 16]
        .copy_from_slice(&((count << 32) | u64::from(R_X86_64_PC8)).to_le_bytes());
...
fn rejects_nonwritable_target() {
    ...
    let output = run(&[&bad]);
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains("PC8 target"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_result_outside_signed_8_bits() {
    let dir = temp("overflow");
    let image = fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
    let rela = rela_offset(&bytes);
    make_same_image(&mut bytes, rela, 128);
    let bad = dir.join("overflow.so");
    fs::write(&bad, bytes).unwrap();
    let output = run(&[&bad]);
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains("does not fit i8"));
    fs::remove_dir_all(dir).unwrap();
}
