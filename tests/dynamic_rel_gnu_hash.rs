...
#[test]
fn gnu_hash_only_dynamic_rel_matches_gnu_readelf() {
    if !tool_available("as") || !tool_available("ld") || !tool_available("readelf") {
        return;
    }
    let dir = temp_dir("dynrel-gnu-hash");
...
fn malformed_gnu_hash_bucket_is_rejected() {
    ...
    let dir = temp_dir("dynrel-gnu-hash-bucket");
    let shared = build_shared(&dir);
...
fn overflowing_dt_gnu_hash_virtual_range_is_rejected() {
    ...
    let dir = temp_dir("dynrel-gnu-hash-overflow");
    let shared = build_shared(&dir);
...
#[test]
fn gnu_hash_fixture_still_has_dynamic_rel() {
    if !tool_available("as") || !tool_available("ld") {
        return;
    }
    let dir = temp_dir("dynrel-gnu-hash-rel");
    let shared = build_shared(&dir);
    let bytes = fs::read(&shared).unwrap();
    let rel_address = read_u64(&bytes, dynamic_value_offset(&bytes, DT_REL));
    assert!(virtual_to_file(&bytes, rel_address) < bytes.len());
    let _ = fs::remove_dir_all(dir);
}
