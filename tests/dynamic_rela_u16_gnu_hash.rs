    ...
    loop {
        ...
        if info as u32 == 1 {
            bytes[cursor + 8..cursor + 16]
                .copy_from_slice(&(((info >> 32) << 32) | u64::from(R_X86_64_16)).to_le_bytes());
            break;
        }
        cursor += 24;
    }
...
fn run_tool(image: &Path) -> Output {
    Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-u16"))
        .args(["--load-bias", "0"])
        .arg(image)
        .output()
        .unwrap()
...
fn accepts_gnu_hash_only_image_and_gnu_readelf_recognizes_relocation() {
    ...
    assert!(reloc_text.contains("R_X86_64_16"), "{reloc_text}");
    ...
    assert!(String::from_utf8_lossy(&output.stdout).contains("Validated R_X86_64_16 relocations"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_malformed_gnu_hash_bloom_count() {
    let dir = temp_dir("u16-gnu-hash-bloom");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
