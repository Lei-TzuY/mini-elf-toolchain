...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-empty-search-paths-{}-{stamp}",
        std::process::id()
    ));
...
#[test]
fn empty_runpath_component_resolves_process_cwd_first() {
    let dir = temp_dir();
    ...
    let cwd_dependency = build_shared(&dir, &dir, "dep", "public_api", &[], &[], None);
    build_shared(&dir, &later_dir, "dep", "public_api", &[], &[], None);
    let search_path = format!(":{}", later_dir.display());
    ...
    assert!(dynamic.contains(&format!("[:{}]", later_dir.display())));
    ...
    assert!(stdout.contains(&format!("file={}", cwd_dependency.display())));

    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn empty_rpath_component_is_inherited_as_process_cwd() {
    let dir = temp_dir();
    let root_dir = dir.join("root");
    ...
    let leaf = build_shared(&dir, &dir, "leaf", "public_api", &[], &[], None);
    ...
    let search_path = format!("{}:", middle_dir.display());
    let root = build_shared(
        &dir,
        &root_dir,
        "root",
        ...
        &["middle"],
        &[&middle_dir],
        Some((&search_path, false)),
    );

    let dynamic = run(Command::new("readelf").arg("-dW").arg(&root));
    let dynamic = String::from_utf8(dynamic.stdout).unwrap();
    assert!(dynamic.contains("(RPATH)"));
    assert!(dynamic.contains(&format!("[{}:]", middle_dir.display())));
    ...
    assert!(stdout.contains(&format!("file={}", leaf.display())));
