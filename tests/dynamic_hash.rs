...
fn dynamic_hash_value_offset(bytes: &[u8]) -> usize {
    ...
    panic!("shared object should contain DT_HASH")
}

fn hash_file_offset(bytes: &[u8]) -> usize {
    let hash_address = read_u64(bytes, dynamic_hash_value_offset(bytes));
    for (segment_type, file_offset, virtual_address, file_size) in program_headers(bytes) {
        if segment_type != 1 {
            continue;
        }
        let end = virtual_address + file_size;
        if hash_address >= virtual_address && hash_address + 8 <= end {
            return (file_offset + hash_address - virtual_address) as usize;
        }
    }
    panic!("DT_HASH should be backed by PT_LOAD")
}
...
#[test]
fn sysv_hash_symbol_count_matches_gnu_dynsym() {
    if !tool_available("as") || !tool_available("ld") || !tool_available("readelf") {
        return;
    }
    let dir = temp_dir("dynhash-gnu");
    ...
    let ours = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynhash"))
        .arg(&shared)
        .output()
        .unwrap();
    assert!(
    ...
    assert!(ours_text.contains("SysV DT_HASH"), "{ours_text}");
    assert!(
        ours_text.contains(&format!("dynamic symbols: {symbol_count}")),
        "ours={ours_text}\ngnu={gnu_text}"
    );
    assert!(ours_text.contains("Buckets:"), "{ours_text}");
    assert!(ours_text.contains("Chains:"), "{ours_text}");
    let _ = fs::remove_dir_all(dir);
}

#[test]
fn malformed_later_hash_bucket_keeps_stdout_atomic() {
    if !tool_available("as") || !tool_available("ld") {
        return;
    }
    let dir = temp_dir("dynhash-atomic");
    let good = build_shared(&dir);
    let bad = dir.join("bad.so");
    let mut bytes = fs::read(&good).unwrap();
    let hash = hash_file_offset(&bytes);
    let chain_count = read_u32(&bytes, hash + 4);
    bytes[hash + 8..hash + 12].copy_from_slice(&chain_count.to_le_bytes());
    fs::write(&bad, bytes).unwrap();

    let output = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynhash"))
        .arg(&good)
        .arg(&bad)
        .output()
        .unwrap();
    ...
    assert!(stderr.contains("outside dynamic-symbol count"), "{stderr}");
    let _ = fs::remove_dir_all(dir);
}

#[test]
fn overflowing_dt_hash_virtual_range_is_rejected() {
    if !tool_available("as") || !tool_available("ld") {
        return;
    }
    let dir = temp_dir("dynhash-overflow");
    let shared = build_shared(&dir);
    let bad = dir.join("overflow.so");
    let mut bytes = fs::read(&shared).unwrap();
    let value_offset = dynamic_hash_value_offset(&bytes);
    bytes[value_offset..value_offset + 8].copy_from_slice(&(u64::MAX - 3).to_le_bytes());
    fs::write(&bad, bytes).unwrap();
