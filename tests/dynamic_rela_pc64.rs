...
fn rela_offset(b: &[u8]) -> usize {
    ...
    loop {
        if u64at(b, o + 8) as u32 == R_X86_64_PC64 {
            return o;
        }
        o += 24
    }
...
fn make_same_image(b: &mut [u8], r: usize, delta: u64, addend: i64) {
    let info = u64at(b, r + 8);
    let si = info >> 32;
    let target = u64at(b, r);
    let st = map_vaddr(b, dynamic_tag(b, DT_SYMTAB));
    let s = st + usize::try_from(si).unwrap() * 24;
    b[s + 6..s + 8].copy_from_slice(&1u16.to_le_bytes());
    b[s + 8..s + 16].copy_from_slice(&target.checked_add(delta).unwrap().to_le_bytes());
    b[r + 16..r + 24].copy_from_slice(&addend.to_le_bytes());
}

#[test]
fn validates_gnu_pc64_external_relocation() {
    let d = temp("good");
    let image = fixture(&d);
    ...
    assert!(String::from_utf8_lossy(&re.stdout).contains("R_X86_64_PC64"));
    let o = run(&[&image]);
...
fn validates_negative_same_image_result() {
    ...
    let p = d.join("negative.so");
    fs::write(&p, b).unwrap();
    let o = run(&[&p]);
    assert!(o.status.success(), "{}", String::from_utf8_lossy(&o.stderr));
    assert!(String::from_utf8_lossy(&o.stdout).contains("result=-1"));
    fs::remove_dir_all(d).unwrap();
}
...
fn rejects_invalid_symbol_index() {
    ...
    b[r + 8..r + 16].copy_from_slice(&((n << 32) | u64::from(R_X86_64_PC64)).to_le_bytes());
    let p = d.join("bad.so");
    fs::write(&p, b).unwrap();
    let o = run(&[&p]);
...
fn rejects_nonwritable_target() {
    ...
    let o = run(&[&p]);
    assert!(!o.status.success());
    assert!(String::from_utf8_lossy(&o.stderr).contains("PC64 target"));
    fs::remove_dir_all(d).unwrap();
}

#[test]
fn rejects_result_outside_signed_64_bits() {
    let d = temp("overflow");
    let image = fixture(&d);
    let mut b = fs::read(&image).unwrap();
    let r = rela_offset(&b);
    make_same_image(&mut b, r, 1, i64::MAX);
    let p = d.join("overflow.so");
    fs::write(&p, b).unwrap();
    let o = run(&[&p]);
    assert!(!o.status.success());
    assert!(String::from_utf8_lossy(&o.stderr).contains("does not fit i64"));
    fs::remove_dir_all(d).unwrap();
}
