...
#[test]
fn dynamic_rel_matches_gnu_readelf_dynamic_view() {
    if !tool_available("as") || !tool_available("ld") || !tool_available("readelf") {
        return;
    }
    let dir = temp_dir("dynrel-gnu");
...
#[test]
fn malformed_later_rel_entry_size_keeps_stdout_atomic() {
    if !tool_available("as") || !tool_available("ld") {
        return;
    }
    let dir = temp_dir("dynrel-atomic");
...
#[test]
fn incomplete_rel_tag_tuple_is_rejected() {
    if !tool_available("as") || !tool_available("ld") {
        return;
    }
    let dir = temp_dir("dynrel-incomplete");
    let shared = build_rel_shared(&dir);
    let bad = dir.join("incomplete.so");
    let mut bytes = fs::read(&shared).unwrap();
    let relent = dynamic_value_offset(&bytes, 19) - 8;
    bytes[relent..relent + 8].copy_from_slice(&0x6000_0000_i64.to_le_bytes());
    fs::write(&bad, bytes).unwrap();
    ...
    assert!(String::from_utf8_lossy(&output.stderr).contains("must provide DT_REL"));
    let _ = fs::remove_dir_all(dir);
}

#[test]
fn overflowing_dt_rel_virtual_range_is_rejected() {
    if !tool_available("as") || !tool_available("ld") {
        return;
    }
    let dir = temp_dir("dynrel-overflow");
    let shared = build_rel_shared(&dir);
    let bad = dir.join("overflow.so");
    let mut bytes = fs::read(&shared).unwrap();
    let rel = dynamic_value_offset(&bytes, 17);
    bytes[rel..rel + 8].copy_from_slice(&(u64::MAX - 7).to_le_bytes());
    fs::write(&bad, bytes).unwrap();
