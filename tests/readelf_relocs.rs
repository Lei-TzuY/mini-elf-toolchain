...
fn assemble_sample(dir: &std::path::Path) -> std::path::PathBuf {
    ...
    let output = Command::new("as").arg("-o").arg(&object).arg(&assembly).output().unwrap();
...
fn relocation_core_facts_match_gnu_readelf() {
    if !tool_available("as") || !tool_available("readelf") { return; }
    let dir = temp_dir("relocs-gnu");
    let object = assemble_sample(&dir);
    let ours = Command::new(env!("CARGO_BIN_EXE_mini-elf-relocs")).arg(&object).output().unwrap();
    assert!(ours.status.success(), "{}", String::from_utf8_lossy(&ours.stderr));
    let gnu = Command::new("readelf").arg("-rW").arg(&object).output().unwrap();
...
fn malformed_later_relocation_table_keeps_stdout_atomic() {
    ...
    let rela = (0..shnum).find_map(|index| {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_mini-elf-relocs")).arg(&good).arg(&bad).output().unwrap();
...
fn overflowing_relocation_section_range_is_rejected() {
    ...
    fs::write(&bad, bytes).unwrap();
    let output = Command::new(env!("CARGO_BIN_EXE_mini-elf-relocs")).arg(&bad).output().unwrap();
    assert!(!output.status.success());
