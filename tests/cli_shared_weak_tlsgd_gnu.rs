...
fn assert_named_weak_tlsgd_metadata(shared: &Path) {
...
#[test]
fn versioned_weak_tlsgd_matches_gnu_and_preserves_version_node_requirement() {
    if !have_tools() {
        return;
    }
...
"#,
        ...
        for shared in [&mini, &gnu] {
            ...
            assert!(
                bound.success(),
                "named-version weak TLSGD bound runtime returned {bound} for {}",
                shared.display()
            );
