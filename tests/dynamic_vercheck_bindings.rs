...
fn temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-vercheck-bindings-{label}-{}-{stamp}",
        std::process::id()
    ));
...
#[test]
fn binds_version_indices_to_checked_dynamic_symbol_names() {
    ...
    assert!(
        ours.contains("name=exported") && ours.contains("version=VERS_LOCAL"),
        "{ours}"
    );
    assert!(
        ours.contains("name=dep") && ours.contains("version=VERS_DEP"),
        "{ours}"
    );
    ...
    assert!(readelf.contains("exported@@VERS_LOCAL"), "{readelf}");
    assert!(readelf.contains("dep@VERS_DEP"), "{readelf}");
}

#[test]
fn rejects_non_elf64_dt_syment() {
    let dir = temp_dir("syment");
    let good = build_fixture(&dir);
    let bad = dir.join("bad-syment.so");
    let mut bytes = fs::read(&good).unwrap();
    patch_dynamic_tag_value(&mut bytes, DT_SYMENT, 16).unwrap();
...
#[test]
fn rejects_dynsym_virtual_range_overflow_atomically() {
    let dir = temp_dir("overflow");
    let good = build_fixture(&dir);
    let bad = dir.join("overflow.so");
    let mut bytes = fs::read(&good).unwrap();
    patch_dynamic_tag_value(&mut bytes, DT_SYMTAB, u64::MAX - 7).unwrap();
    fs::write(&bad, bytes).unwrap();
