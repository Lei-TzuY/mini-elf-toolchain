...
fn fixture(dir: &Path) -> PathBuf {
    ...
    b[r + 8..r + 16]
        .copy_from_slice(&(((info >> 32) << 32) | R_X86_64_TPOFF32 as u64).to_le_bytes());
...
fn rejects_bad_symbol_index() {
    let d = temp("tpoff32-index");
    let so = fixture(&d);
    let mut b = fs::read(&so).unwrap();
    let r = rela_of_type(&b, R_X86_64_TPOFF32);
    let n = count(&b);
...
fn rejects_signed_result_overflow() {
    let d = temp("tpoff32-overflow");
    let so = fixture(&d);
    let mut b = fs::read(&so).unwrap();
    let r = rela_of_type(&b, R_X86_64_TPOFF32);
    let info = u64at(&b, r + 8);
    let si = (info >> 32) as usize;
    let s = map(&b, tag(&b, 6)) + si * 24;
...
fn rejects_runtime_target_overflow() {
    let d = temp("tpoff32-runtime-overflow");
    let so = fixture(&d);
