...
fn run(inputs: &[&Path]) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-size32"));
    for input in inputs {
        command.arg(input);
    }
...
fn rela_offset(b: &[u8]) -> usize {
    ...
    loop {
        if u64at(b, o + 8) as u32 == R_X86_64_SIZE32 {
            return o;
        }
        o += 24;
    }
...
#[test]
fn validates_gnu_size32_external_relocation() {
    let d = temp("good");
    let image = fixture(&d);
    ...
    assert!(String::from_utf8_lossy(&re.stdout).contains("R_X86_64_SIZE32"));
    let o = run(&[&image]);
...
fn rejects_invalid_symbol_index() {
    ...
    b[r + 8..r + 16].copy_from_slice(&((n << 32) | u64::from(R_X86_64_SIZE32)).to_le_bytes());
    let p = d.join("bad.so");
    fs::write(&p, b).unwrap();
    let o = run(&[&p]);
...
fn rejects_nonwritable_target() {
    ...
    let o = run(&[&p]);
    assert!(!o.status.success());
    assert!(String::from_utf8_lossy(&o.stderr).contains("SIZE32 target"));
    fs::remove_dir_all(d).unwrap();
}

#[test]
fn rejects_size_plus_addend_outside_unsigned_32_bits() {
    let d = temp("overflow");
    let image = fixture(&d);
    let mut b = fs::read(&image).unwrap();
    let r = rela_offset(&b);
    make_same_image(&mut b, r, u64::from(u32::MAX) + 1, 0);
    let p = d.join("overflow.so");
    fs::write(&p, b).unwrap();
    let o = run(&[&p]);
    assert!(!o.status.success());
    assert!(String::from_utf8_lossy(&o.stderr).contains("does not fit u32"));
    fs::remove_dir_all(d).unwrap();
}
