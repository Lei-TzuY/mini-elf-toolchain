...
fn rejects_unsigned_8_underflow_and_overflow() {
    ...
    let overflow =
        evaluate_relocation(&relocation(R_X86_64_8, 1), u64::from(u8::MAX), 0).unwrap_err();
...
fn rejects_pc8_positive_and_negative_overflow() {
    let positive =
        evaluate_relocation(&relocation(R_X86_64_PC8, i64::from(i8::MAX) + 1), 0, 0).unwrap_err();
    let negative =
        evaluate_relocation(&relocation(R_X86_64_PC8, i64::from(i8::MIN) - 1), 0, 0).unwrap_err();
