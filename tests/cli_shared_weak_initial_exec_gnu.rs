...
const WEAK_TLS: &str = "mini_elf_optional_weak_ie_tls_332";
...
fn temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-weak-ie-{label}-{}-{nonce}",
        std::process::id()
    ));
...
fn unversioned_consumer(dir: &Path) -> PathBuf {
    assemble(
        dir,
        "consumer",
        &format!(
...
.text
.globl read_optional_tls_ie
.type read_optional_tls_ie,@function
.weak {WEAK_TLS}
.type {WEAK_TLS},@tls_object
read_optional_tls_ie:
    mov {WEAK_TLS}@gottpoff(%rip), %rax
    mov %fs:(%rax), %rax
    ret
.size read_optional_tls_ie, .-read_optional_tls_ie

.globl write_optional_tls_ie
.type write_optional_tls_ie,@function
write_optional_tls_ie:
    mov {WEAK_TLS}@gottpoff(%rip), %rax
    mov %rdi, %fs:(%rax)
    ret
.size write_optional_tls_ie, .-write_optional_tls_ie
"#
        ),
    )
}
...
fn versioned_weak_consumer(dir: &Path) -> PathBuf {
    assemble(
        ...
        r#".section .note.GNU-stack,"",@progbits
.text
.globl read_provider_tls_ie
.type read_provider_tls_ie,@function
.weak provider_tls
.type provider_tls,@tls_object
.symver provider_tls,provider_tls@VERS_1
read_provider_tls_ie:
    mov provider_tls@gottpoff(%rip), %rax
    mov %fs:(%rax), %rax
    ret
.size read_provider_tls_ie, .-read_provider_tls_ie

.globl write_provider_tls_ie
.type write_provider_tls_ie,@function
write_provider_tls_ie:
    mov provider_tls@gottpoff(%rip), %rax
    mov %rdi, %fs:(%rax)
    ret
.size write_provider_tls_ie, .-write_provider_tls_ie
"#,
    )
}
...
fn assert_weak_ie_metadata(shared: &Path, symbol: &str) {
    ...
    assert!(
        relocations
            .lines()
            .any(|line| line.contains("R_X86_64_TPOFF64") && line.contains(symbol)),
        "{} dynamic relocations:\n{relocations}",
        shared.display()
    );
    assert!(
        ...
        "{} weak initial-exec access must stay on TPOFF64 rather than another TLS model:\n{relocations}",
        shared.display()
    );
}

#[test]
fn unversioned_weak_initial_exec_matches_gnu_and_zero_or_binds_at_load_time() {
    if !have_tools() {
        return;
    }

    let dir = temp_dir("unversioned");
    let object = unversioned_consumer(&dir);
    ...
    assert!(
        input.contains("R_X86_64_GOTTPOFF") && input.contains(WEAK_TLS),
        "{input}"
    );
    ...
    for shared in [&mini, &gnu] {
        assert_weak_ie_metadata(shared, WEAK_TLS);
    }
...
int main(int argc, char **argv) {
    ...
    if (!dlsym(handle, "read_optional_tls_ie")) return 202;
    if (!dlsym(handle, "write_optional_tls_ie")) return 203;
...
int main(int argc, char **argv) {{
    ...
    read_fn read_tls = (read_fn)dlsym(handle, "read_optional_tls_ie");
    write_fn write_tls = (write_fn)dlsym(handle, "write_optional_tls_ie");
...
"#
        ...
        for shared in [&mini, &gnu] {
            ...
            assert!(
                absent.success(),
                "weak initial-exec absent-symbol load returned {absent} for {}",
                shared.display()
            );
            ...
            assert!(
                bound.success(),
                "weak initial-exec host-binding runtime returned {bound} for {}",
                shared.display()
            );
        }
    }
...
#[test]
fn named_version_weak_initial_exec_preserves_version_node_requirement() {
    if !have_tools() {
        return;
    }
    ...
    let object = versioned_weak_consumer(&dir);
    ...
    let input = String::from_utf8_lossy(&input.stdout);
    assert!(
        input.contains("R_X86_64_GOTTPOFF") && input.contains("provider_tls@VERS_1"),
        "{input}"
    );
    ...
    for shared in [&mini, &gnu] {
        assert_weak_ie_metadata(shared, "provider_tls@VERS_1");
    }
...
int main(int argc, char **argv) {
    ...
    if (argv[2][0] == '1') {
        ...
        if (!dlsym(handle, "read_provider_tls_ie")) return 222;
        if (!dlsym(handle, "write_provider_tls_ie")) return 223;
...
int main(int argc, char **argv) {
    ...
    read_fn read_tls = (read_fn)dlsym(handle, "read_provider_tls_ie");
    write_fn write_tls = (write_fn)dlsym(handle, "write_provider_tls_ie");
...
"#,
        ...
        for shared in [&mini, &gnu] {
            ...
            assert!(
                bound.success(),
                "named-version weak initial-exec bound runtime returned {bound} for {}",
                shared.display()
            );
            ...
            assert!(
                missing.success(),
                "matching VERS_1 with absent weak TLS symbol must remain loadable for {}",
                shared.display()
            );
            ...
            assert!(
                wrong.success(),
                "missing required VERS_1 must reject loading for {}",
                shared.display()
            );
        }
    }
