...
fn run_tool(inputs: &[&Path], bias: &str) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-u8"));
...
fn rela_offset(bytes: &[u8]) -> usize {
    ...
    loop {
        if read_u64(bytes, cursor + 8) as u32 == R_X86_64_8 {
            return cursor;
        }
        cursor += 24;
    }
...
#[test]
fn validates_gnu_recognized_8_relocation() {
    let dir = temp_dir("u8-good");
    ...
    assert!(text.contains("R_X86_64_8"), "{text}");
    ...
    assert!(stdout.contains("Validated R_X86_64_8 relocations"));
...
fn applies_negative_addend_with_unsigned_range_check() {
    let dir = temp_dir("u8-negative");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
    let rela = rela_offset(&bytes);
    let addend = read_i64(&bytes, rela + 16) - 1;
    assert!(addend < 0);
    ...
    assert!(String::from_utf8_lossy(&output.stdout).contains("result=127"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_invalid_symbol_index() {
    let dir = temp_dir("u8-index");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
    let rela = rela_offset(&bytes);
    let count = symbol_count(&bytes);
    bytes[rela + 8..rela + 16]
        .copy_from_slice(&((count << 32) | u64::from(R_X86_64_8)).to_le_bytes());
...
#[test]
fn rejects_result_outside_unsigned_8_bits() {
    let dir = temp_dir("u8-overflow");
    let image = build_fixture(&dir);
    let output = run_tool(&[&image], "0x100");
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains("does not fit unsigned 8 bits"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn malformed_later_input_keeps_stdout_atomic() {
    let dir = temp_dir("u8-atomic");
