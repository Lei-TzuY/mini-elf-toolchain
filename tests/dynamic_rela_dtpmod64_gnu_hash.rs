...
fn run_tool(image: &Path) -> Output {
    Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-dtpmod64"))
        .args(["--load-bias", "0x70000000", "--module-id", "7"])
        .arg(image)
        .output()
        .unwrap()
}
...
fn accepts_gnu_hash_only_image_and_gnu_readelf_recognizes_relocation() {
    let dir = temp_dir("dtpmod64-gnu-hash");
    ...
    assert!(reloc_text.contains("R_X86_64_DTPMOD64"), "{reloc_text}");
    ...
    assert!(stdout.contains("Validated R_X86_64_DTPMOD64 relocations"));
    assert!(stdout.contains("result-module-id=7"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_malformed_gnu_hash_bloom_count() {
    let dir = temp_dir("dtpmod64-gnu-hash-bloom");
    let image = build_fixture(&dir);
