...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-gnu-hash-resolve-{}-{stamp}",
        std::process::id()
    ));
...
fn build_gnu_hash_so(dir: &Path, stem: &str) -> PathBuf {
...
fn tool() -> &'static str {
    env!("CARGO_BIN_EXE_mini-elf-gnu-hash-resolve")
}
...
fn resolves_first_eligible_definition_in_explicit_input_order() {
    let dir = temp_dir();
    let first = build_gnu_hash_so(&dir, "first");
    let second = build_gnu_hash_so(&dir, "second");
...
fn returns_not_found_when_no_input_defines_symbol() {
    ...
    assert_eq!(
        ...
        "GNU external resolve: symbol=missing_api not-found\n"
    );
...
fn malformed_earlier_input_fails_without_stdout() {
    let dir = temp_dir();
    let bad = build_gnu_hash_so(&dir, "bad");
    let good = build_gnu_hash_so(&dir, "good");
