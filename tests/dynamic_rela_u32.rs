...
fn run_tool(inputs: &[&Path], bias: &str) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-u32"));
...
fn u32_rela_offset(bytes: &[u8]) -> usize {
    let rela = dynamic_tag(bytes, DT_RELA);
    let mut cursor = map_vaddr(bytes, rela);
    loop {
        let info = read_u64(bytes, cursor + 8);
        if info as u32 == R_X86_64_32 {
            return cursor;
        }
        cursor += 24;
    }
...
#[test]
fn validates_gnu_u32_relocation() {
    let dir = temp_dir("u32-good");
    ...
    assert!(relocations.contains("R_X86_64_32"), "{relocations}");
    ...
    assert!(stdout.contains("Validated R_X86_64_32 relocations"));
...
fn applies_negative_rela_addend() {
    let dir = temp_dir("u32-negative-addend");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
...
fn rejects_out_of_range_dynamic_symbol_index() {
    let dir = temp_dir("u32-symbol-index");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
    let rela = u32_rela_offset(&bytes);
    let count = symbol_count(&bytes);
    bytes[rela + 8..rela + 16]
        .copy_from_slice(&((count << 32) | u64::from(R_X86_64_32)).to_le_bytes());
