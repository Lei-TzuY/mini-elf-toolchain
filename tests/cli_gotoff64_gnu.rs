...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-cli-gotoff64-{}-{nonce}",
        std::process::id()
    ));
...
#[test]
fn cli_links_gnu_gotoff64_against_synthetic_got_base() {
    ...
    assert!(
        relocation_stdout.contains("R_X86_64_GOTOFF64"),
        "assembler did not produce GOTOFF64 relocation: {relocation_stdout}"
    );
