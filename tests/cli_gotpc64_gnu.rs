...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-cli-gotpc64-{}-{nonce}",
        std::process::id()
    ));
...
#[test]
fn cli_links_gnu_gotpc64_against_synthetic_got_base() {
    ...
    assert!(
        relocation_stdout.contains("R_X86_64_GOTPC64"),
        "assembler did not produce GOTPC64 relocation: {relocation_stdout}"
    );
    ...
    #[cfg(target_os = "linux")]
    {
        let status = Command::new(dir.join("ours.out"))
            .status()
            .expect("execute linked GOTPC64 ELF");
        assert!(status.success(), "GOTPC64 executable returned {status}");
    }
