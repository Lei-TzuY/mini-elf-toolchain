...
#[test]
fn weak_function_plt_call_remains_fail_closed() {
    if !command_reports("as", "GNU assembler") {
        return;
    }

    let dir = temp_dir("plt-reject");
    let object = assemble(
        &dir,
        "consumer-call",
        r#".section .note.GNU-stack,"",@progbits
...
.type optional_function,@function
call_optional:
    call optional_function@PLT
    ret
...
"#,
    ...
    assert!(
        stderr.contains("unsupported binding") || stderr.contains("weak"),
        "{stderr}"
    );
