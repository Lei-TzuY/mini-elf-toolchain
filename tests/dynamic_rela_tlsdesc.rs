...
fn run(paths: &[&Path], bias: &str) -> Output {
    let mut c = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-tlsdesc"));
...
#[test]
fn validates_gnu_tlsdesc() {
    let d = temp("tlsdesc-good");
    let so = fixture(&d);
    ...
    assert!(gt.contains("R_X86_64_TLSDESC"), "{gt}");
    let o = run(&[&so], "0x70000000");
    assert!(o.status.success(), "{}", String::from_utf8_lossy(&o.stderr));
    assert!(String::from_utf8_lossy(&o.stdout).contains("Validated R_X86_64_TLSDESC"));
    fs::remove_dir_all(d).unwrap();
}
#[test]
fn rejects_bad_symbol_index() {
    let d = temp("tlsdesc-index");
    let so = fixture(&d);
    let mut b = fs::read(&so).unwrap();
    let r = rela(&b);
    let n = count(&b);
    b[r + 8..r + 16].copy_from_slice(&((n << 32) | R_X86_64_TLSDESC as u64).to_le_bytes());
...
#[test]
fn rejects_nonwritable_descriptor() {
    let d = temp("tlsdesc-target");
    let so = fixture(&d);
    let mut b = fs::read(&so).unwrap();
    let r = rela(&b);
    let a = exec_addr(&b);
    ...
    let o = run(&[&bad], "0");
    assert!(!o.status.success());
    assert!(String::from_utf8_lossy(&o.stderr).contains("descriptor target"));
    fs::remove_dir_all(d).unwrap();
}
#[test]
fn rejects_tls_offset_underflow() {
    let d = temp("tlsdesc-underflow");
    let so = fixture(&d);
    let mut b = fs::read(&so).unwrap();
    let r = rela(&b);
    b[r + 16..r + 24].copy_from_slice(&(-1i64).to_le_bytes());
    let bad = d.join("bad.so");
    fs::write(&bad, b).unwrap();
    let o = run(&[&bad], "0");
    assert!(!o.status.success());
    assert!(String::from_utf8_lossy(&o.stderr).contains("TLS offset does not fit unsigned 64-bit"));
    fs::remove_dir_all(d).unwrap();
}
#[test]
fn rejects_runtime_descriptor_overflow() {
    let d = temp("tlsdesc-runtime");
    let so = fixture(&d);
    let o = run(&[&so], "0xffffffffffffffff");
    assert!(!o.status.success());
    assert!(String::from_utf8_lossy(&o.stderr).contains("runtime descriptor"));
    fs::remove_dir_all(d).unwrap();
}
#[test]
fn malformed_later_input_keeps_stdout_atomic() {
    let d = temp("tlsdesc-atomic");
    let so = fixture(&d);
    let bad = d.join("bad.so");
