...
const TLS_NAME: &str = "mini_elf_protected_tls";
const INITIAL: u64 = 0x1111_2222_3333_4444;
const HOST_INITIAL: u64 = 0x9999_aaaa_bbbb_cccc;
...
fn temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-protected-tls-{label}-{}-{nonce}",
        std::process::id()
    ));
...
fn protected_tls_source(body: &str) -> String {
    format!(
...
fn assemble_gnu_model(dir: &Path, stem: &str, body: String) -> PathBuf {
    assemble_source(dir, stem, &protected_tls_source(&body))
}

fn build_mini(dir: &Path, object: &Path) -> PathBuf {
    let shared = dir.join("libmini-protected-tls.so");
    let output = Command::new(env!("CARGO_BIN_EXE_mini-elf-toolchain"))
        .args(["link", "-o"])
        .arg(&shared)
        .args(["--shared", "--soname", "libmini-protected-tls.so"])
        .arg(object)
        .output()
        .unwrap();
    assert!(
...
fn build_gnu(dir: &Path, stem: &str, object: &Path) -> PathBuf {
    let shared = dir.join(format!("libgnu-protected-{stem}.so"));
...
fn assert_protected_tls_symbol(path: &Path) {
...
fn assert_mini_metadata(path: &Path) {
    assert_protected_tls_symbol(path);
...
fn assert_gnu_model_metadata(
    ...
    for relocation in expected_relocations {
        assert!(
            relocations.lines().any(|line| line.contains(relocation)),
            "{} missing {relocation}:\n{relocations}",
            path.display()
        );
    }
...
static void *worker(void *opaque) {{
    ...
    ctx->api.desc_write(UINT64_C(0x3333333333333333));
...
int main(int argc, char **argv) {{
    ...
    uint64_t *tls = (uint64_t *)dlsym(handle, "{TLS_NAME}");
    if (!api.gd_read || !api.gd_write || !api.ie_read || !api.ie_write ||
        !api.desc_read || !api.desc_write || !tls) return 202;
    if (*tls != UINT64_C(0x{INITIAL:016x}) ||
        all_equal(&api, UINT64_C(0x{INITIAL:016x})) != 0) return 203;

    api.gd_write(UINT64_C(0xaaaaaaaaaaaaaaaa));
    if (*tls != UINT64_C(0xaaaaaaaaaaaaaaaa) ||
        all_equal(&api, UINT64_C(0xaaaaaaaaaaaaaaaa)) != 0) return 204;
    api.ie_write(UINT64_C(0xbbbbbbbbbbbbbbbb));
    if (*tls != UINT64_C(0xbbbbbbbbbbbbbbbb) ||
        all_equal(&api, UINT64_C(0xbbbbbbbbbbbbbbbb)) != 0) return 205;

    struct ctx ctx = {{ api, -1 }};
    pthread_t thread;
    if (pthread_create(&thread, 0, worker, &ctx) != 0) return 206;
    if (pthread_join(thread, 0) != 0) return 207;
    if (ctx.result != 0) return ctx.result;
    if (*tls != UINT64_C(0xbbbbbbbbbbbbbbbb) ||
        all_equal(&api, UINT64_C(0xbbbbbbbbbbbbbbbb)) != 0) return 208;

    return dlclose(handle) == 0 ? 0 : 209;
}}
"#
    )
}
...
int main(int argc, char **argv) {{
    ...
    uint64_t *local_tls = (uint64_t *)dlsym(handle, "{TLS_NAME}");
    uint64_t *global_tls = (uint64_t *)dlsym(RTLD_DEFAULT, "{TLS_NAME}");
    if (!api.gd_read || !api.gd_write || !api.ie_read || !api.ie_write ||
        !api.desc_read || !api.desc_write || !local_tls || !global_tls) return 222;
    if (global_tls != &{TLS_NAME} || local_tls == &{TLS_NAME}) return 223;
    if ({TLS_NAME} != UINT64_C(0x{HOST_INITIAL:016x}) ||
        *local_tls != UINT64_C(0x{INITIAL:016x}) ||
        all_equal(&api, UINT64_C(0x{INITIAL:016x})) != 0) return 224;

    api.desc_write(UINT64_C(0xdddddddddddddddd));
    if ({TLS_NAME} != UINT64_C(0x{HOST_INITIAL:016x}) ||
        *local_tls != UINT64_C(0xdddddddddddddddd) ||
        all_equal(&api, UINT64_C(0xdddddddddddddddd)) != 0) return 225;

    struct ctx ctx = {{ api, -1 }};
    pthread_t thread;
    if (pthread_create(&thread, 0, worker, &ctx) != 0) return 226;
    if (pthread_join(thread, 0) != 0) return 227;
    if (ctx.result != 0) return ctx.result;

    if ({TLS_NAME} != UINT64_C(0x{HOST_INITIAL:016x}) ||
        *local_tls != UINT64_C(0xdddddddddddddddd) ||
        all_equal(&api, UINT64_C(0xdddddddddddddddd)) != 0) return 228;
    return dlclose(handle) == 0 ? 0 : 229;
}}
"#
    )
}

fn model_runner_source(preempt: bool) -> String {
    if preempt {
        format!(
            r#"#include <dlfcn.h>
#include <pthread.h>
...
static void *worker(void *opaque) {{
    struct ctx *ctx = (struct ctx *)opaque;
    if ({TLS_NAME} != UINT64_C(0x{HOST_INITIAL:016x}) ||
        ctx->read_tls() != UINT64_C(0x{INITIAL:016x})) {{
        ctx->result = 251;
        return 0;
    }}
    ctx->write_tls(UINT64_C(0x6666666666666666));
    if ({TLS_NAME} != UINT64_C(0x{HOST_INITIAL:016x}) ||
        ctx->read_tls() != UINT64_C(0x6666666666666666)) {{
        ctx->result = 252;
        return 0;
    }}
    ctx->result = 0;
...
int main(int argc, char **argv) {{
    ...
    void *handle = dlopen(argv[1], RTLD_NOW | RTLD_LOCAL);
    if (!handle) return 241;
    read_fn read_tls = (read_fn)dlsym(handle, "read_tls");
    write_fn write_tls = (write_fn)dlsym(handle, "write_tls");
    uint64_t *local_tls = (uint64_t *)dlsym(handle, "{TLS_NAME}");
    uint64_t *global_tls = (uint64_t *)dlsym(RTLD_DEFAULT, "{TLS_NAME}");
    if (!read_tls || !write_tls || !local_tls || !global_tls) return 242;
    if (global_tls != &{TLS_NAME} || local_tls == &{TLS_NAME}) return 243;
    if ({TLS_NAME} != UINT64_C(0x{HOST_INITIAL:016x}) ||
        *local_tls != UINT64_C(0x{INITIAL:016x}) ||
        read_tls() != UINT64_C(0x{INITIAL:016x})) return 244;

    write_tls(UINT64_C(0x7777777777777777));
    if ({TLS_NAME} != UINT64_C(0x{HOST_INITIAL:016x}) ||
        *local_tls != UINT64_C(0x7777777777777777) ||
        read_tls() != UINT64_C(0x7777777777777777)) return 245;

    struct ctx ctx = {{ read_tls, write_tls, -1 }};
    pthread_t thread;
    if (pthread_create(&thread, 0, worker, &ctx) != 0) return 246;
    if (pthread_join(thread, 0) != 0) return 247;
    if (ctx.result != 0) return ctx.result;
    if ({TLS_NAME} != UINT64_C(0x{HOST_INITIAL:016x}) ||
        read_tls() != UINT64_C(0x7777777777777777)) return 248;
    return dlclose(handle) == 0 ? 0 : 249;
}}
"#
        )
    } else {
...
static void *worker(void *opaque) {{
    ...
    ctx->write_tls(UINT64_C(0x8888888888888888));
    if (ctx->read_tls() != UINT64_C(0x8888888888888888)) {{
        ctx->result = 272;
        return 0;
    }}
    ctx->result = 0;
...
int main(int argc, char **argv) {{
    ...
    write_fn write_tls = (write_fn)dlsym(handle, "write_tls");
    uint64_t *tls = (uint64_t *)dlsym(handle, "{TLS_NAME}");
    if (!read_tls || !write_tls || !tls) return 262;
    if (*tls != UINT64_C(0x{INITIAL:016x}) ||
        read_tls() != UINT64_C(0x{INITIAL:016x})) return 263;

    write_tls(UINT64_C(0xaaaaaaaaaaaaaaaa));
    if (*tls != UINT64_C(0xaaaaaaaaaaaaaaaa) ||
        read_tls() != UINT64_C(0xaaaaaaaaaaaaaaaa)) return 264;

    struct ctx ctx = {{ read_tls, write_tls, -1 }};
    pthread_t thread;
    if (pthread_create(&thread, 0, worker, &ctx) != 0) return 265;
    if (pthread_join(thread, 0) != 0) return 266;
    if (ctx.result != 0) return ctx.result;
    if (*tls != UINT64_C(0xaaaaaaaaaaaaaaaa) ||
        read_tls() != UINT64_C(0xaaaaaaaaaaaaaaaa)) return 267;
    return dlclose(handle) == 0 ? 0 : 268;
}}
"#
        )
    }
...
#[test]
fn protected_tls_binds_locally_across_dynamic_models_and_matches_gnu() {
    if !have_tools() {
        return;
    }
