    ...
    loop {
        ...
        if info as u32 == 1 {
            ...
            bytes[cursor + 8..cursor + 16]
                .copy_from_slice(&((symbol << 32) | u64::from(R_X86_64_32S)).to_le_bytes());
            break;
        }
        cursor += 24;
    }
...
fn rela_offset(bytes: &[u8]) -> usize {
    ...
    loop {
        if read_u64(bytes, cursor + 8) as u32 == R_X86_64_32S {
            return cursor;
        }
        cursor += 24;
    }
...
fn rejects_invalid_symbol_index() {
    ...
    bytes[rela + 8..rela + 16]
        .copy_from_slice(&((count << 32) | u64::from(R_X86_64_32S)).to_le_bytes());
