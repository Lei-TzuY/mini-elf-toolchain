...
fn temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!("mini-elf-toolchain-{label}-{}-{stamp}", std::process::id()));
...
fn build_fixture(dir: &Path) -> PathBuf {
    ...
    assert!(Command::new("as").args(["-o", obj.to_str().unwrap(), asm.to_str().unwrap()]).status().unwrap().success());
    assert!(Command::new("ld").args(["-shared", "--hash-style=sysv", "-o", image.to_str().unwrap(), obj.to_str().unwrap()]).status().unwrap().success());
    ...
    loop {
        ...
        if info as u32 == 1 {
            let symbol = info >> 32;
            bytes[cursor + 8..cursor + 16].copy_from_slice(&((symbol << 32) | u64::from(R_X86_64_32S)).to_le_bytes());
            break;
        }
        cursor += 24;
    }
...
fn run_tool(inputs: &[&Path], bias: &str) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-i32"));
...
fn program_headers(bytes: &[u8]) -> Vec<(u32, u32, u64, u64, u64)> {
    ...
    (0..phnum).map(|i| {
        ...
        (read_u32(bytes, o), read_u32(bytes, o + 4), read_u64(bytes, o + 8), read_u64(bytes, o + 16), read_u64(bytes, o + 32))
    }).collect()
}

fn map_vaddr(bytes: &[u8], address: u64) -> usize {
    for (kind, _, offset, vaddr, filesz) in program_headers(bytes) {
        if kind == PT_LOAD && address >= vaddr && address < vaddr + filesz { return (offset + address - vaddr) as usize; }
    }
    panic!("address {address:#x} is not file backed");
}

fn dynamic_tag(bytes: &[u8], wanted: i64) -> u64 {
    let dynamic = program_headers(bytes).into_iter().find(|h| h.0 == PT_DYNAMIC).unwrap();
...
fn rela_offset(bytes: &[u8]) -> usize {
    ...
    loop {
        if read_u64(bytes, cursor + 8) as u32 == R_X86_64_32S { return cursor; }
        cursor += 24;
    }
}
...
fn executable_address(bytes: &[u8]) -> u64 {
    program_headers(bytes).into_iter().find(|h| h.0 == PT_LOAD && h.1 & PF_X != 0).unwrap().3
}

#[test]
fn validates_gnu_recognized_32s_relocation() {
    let dir = temp_dir("i32-good");
    let image = build_fixture(&dir);
    let readelf = Command::new("readelf").args(["-rW", image.to_str().unwrap()]).output().unwrap();
    assert!(readelf.status.success());
    let text = String::from_utf8(readelf.stdout).unwrap();
    assert!(text.contains("R_X86_64_32S"), "{text}");
    let output = run_tool(&[&image], "0x10000000");
    assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("Validated R_X86_64_32S relocations"));
    assert!(stdout.contains(":target "));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn applies_negative_addend_with_signed_range_check() {
    let dir = temp_dir("i32-negative");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
    ...
    let output = run_tool(&[&patched], "0x10000000");
    assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
...
fn rejects_invalid_symbol_index() {
    let dir = temp_dir("i32-index");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
    let rela = rela_offset(&bytes);
    let count = symbol_count(&bytes);
    bytes[rela + 8..rela + 16].copy_from_slice(&((count << 32) | u64::from(R_X86_64_32S)).to_le_bytes());
    let bad = dir.join("bad-index.so");
    fs::write(&bad, bytes).unwrap();
    let output = run_tool(&[&bad], "0x10000000");
...
#[test]
fn rejects_result_outside_signed_32_bits() {
    let dir = temp_dir("i32-overflow");
    let image = build_fixture(&dir);
    let output = run_tool(&[&image], "0x80000000");
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains("does not fit signed 32 bits"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn malformed_later_input_keeps_stdout_atomic() {
    let dir = temp_dir("i32-atomic");
    ...
    fs::write(&bad, bytes).unwrap();
    let output = run_tool(&[&good, &bad], "0x10000000");
    assert!(!output.status.success());
