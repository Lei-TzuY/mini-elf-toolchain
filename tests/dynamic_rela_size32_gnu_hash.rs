...
fn temp(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-size32-gnu-{label}-{}-{stamp}",
        std::process::id()
    ));
...
fn run(input: &Path) -> Output {
    Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-size32"))
        .arg(input)
        .output()
        .unwrap()
}
...
#[test]
fn validates_gnu_hash_only_size32() {
    let dir = temp("good");
    ...
    assert!(String::from_utf8_lossy(&relocations.stdout).contains("R_X86_64_SIZE32"));
...
#[test]
fn gnu_hash_fixture_contains_size32_relocation() {
    let dir = temp("reloc");
    let image = fixture(&dir);
    ...
    let found = (0..64).any(|_| {
        let relocation = u64at(&bytes, cursor + 8) as u32 == R_X86_64_SIZE32;
        cursor += 24;
        relocation
    });
    assert!(found);
