...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-sysv-hash-external-lookup-{}-{stamp}",
        std::process::id()
    ));
...
fn build_sysv_hash_so(dir: &Path) -> PathBuf {
...
fn tool() -> &'static str {
    env!("CARGO_BIN_EXE_mini-elf-sysv-hash-external-lookup")
}
...
#[test]
fn resolves_default_visible_defined_global_from_real_sysv_hash_image() {
    let dir = temp_dir();
    let image = build_sysv_hash_so(&dir);
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("SysV external lookup: symbol=public_api index="));
...
fn rejects_hidden_local_and_undefined_matches_without_changing_hash_membership() {
    ...
    assert_eq!(
        ...
        "SysV external lookup: symbol=public_api not-found\n"
    );
...
fn malformed_later_input_keeps_stdout_atomic() {
    let dir = temp_dir();
    let good = build_sysv_hash_so(&dir);
