...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-cli-pltoff64-{}-{nonce}",
        std::process::id()
    ));
...
#[test]
fn cli_links_gnu_pltoff64_in_static_non_preemptible_scope() {
    ...
    assemble(
        &dir,
        "start",
        ".globl _start\n.type _start,@function\n.extern target\n_start:\n  mov $60,%rax\n  xor %rdi,%rdi\n  syscall\n.section .data\n.align 8\npltoff_value:\n  .quad 0\n  .reloc pltoff_value, R_X86_64_PLTOFF64, target\n",
    );
    assemble(
        &dir,
        "defs",
    ...
    assert!(
        relocation_stdout.contains("R_X86_64_PLTOFF64"),
        "assembler did not produce PLTOFF64 relocation: {relocation_stdout}"
    );
    ...
    #[cfg(target_os = "linux")]
    {
        for output in ["ours.out", "gnu.out"] {
            let status = Command::new(dir.join(output))
                .status()
                .expect("execute linked PLTOFF64 ELF");
