...
fn run(paths: &[&Path], bias: &str) -> Output {
    let mut c = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-dtpoff64"));
...
fn rela(b: &[u8]) -> usize {
    ...
    loop {
        if u64at(b, c + 8) as u32 == R_X86_64_DTPOFF64 {
            return c;
        }
        c += 24
    }
...
#[test]
fn validates_gnu_dtpoff64() {
    let d = temp("dtpoff-good");
    let so = fixture(&d);
    ...
    assert!(gt.contains("R_X86_64_DTPOFF64"), "{gt}");
    let o = run(&[&so], "0x70000000");
    assert!(o.status.success(), "{}", String::from_utf8_lossy(&o.stderr));
    let s = String::from_utf8_lossy(&o.stdout);
    assert!(s.contains("Validated R_X86_64_DTPOFF64"));
    assert!(s.contains("result-dtpoff="));
    fs::remove_dir_all(d).unwrap()
}
#[test]
fn rejects_bad_symbol_index() {
    let d = temp("dtpoff-index");
    let so = fixture(&d);
    let mut b = fs::read(&so).unwrap();
    let r = rela(&b);
    let n = count(&b);
    b[r + 8..r + 16].copy_from_slice(&((n << 32) | R_X86_64_DTPOFF64 as u64).to_le_bytes());
...
#[test]
fn rejects_result_underflow() {
    let d = temp("dtpoff-underflow");
    let so = fixture(&d);
    let mut b = fs::read(&so).unwrap();
    let r = rela(&b);
    ...
    b[s + 8..s + 16].copy_from_slice(&0_u64.to_le_bytes());
    let bad = d.join("bad.so");
    fs::write(&bad, b).unwrap();
    let o = run(&[&bad], "0");
    assert!(!o.status.success());
    assert!(String::from_utf8_lossy(&o.stderr).contains("does not fit unsigned 64-bit"));
    fs::remove_dir_all(d).unwrap()
}
#[test]
fn rejects_runtime_target_overflow() {
    let d = temp("dtpoff-overflow");
    let so = fixture(&d);
