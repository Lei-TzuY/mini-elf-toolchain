...
fn temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-crt-startup-{label}-{}-{nonce}",
        std::process::id()
    ));
...
fn reference_startup() -> &'static str {
    r#".text
...
#[test]
#[cfg(target_os = "linux")]
fn synthesized_crt_executes_main_through_loader_binding() {
    if !have_tools() {
        return;
    }
...
"#,
    ...
    assert!(
        relocations.contains("GLOB_DAT") && relocations.contains("main"),
        "{relocations}"
    );
    ...
    assert_eq!(
        status.code(),
        Some(73),
        "synthesized CRT must transfer main's return value through glibc: {status}"
    );
...
#[test]
#[cfg(target_os = "linux")]
fn synthesized_crt_preserves_glibc_lifecycle_execution() {
    if !have_tools() {
        return;
    }
    let Some(interpreter) = dynamic_linker() else {
    ...
    let startup = assemble(&dir, "reference-startup", reference_startup());
    let ours = link_mini(&dir, &object, &interpreter, &libc);

    let gnu = dir.join("gnu-crt");
    ...
    for image in [&ours, &gnu] {
        ...
        assert_eq!(
            status.code(),
            Some(43),
            "{} must execute preinit -> init -> main -> fini: {status}",
            image.display()
        );
    }
...
#[test]
#[cfg(target_os = "linux")]
fn synthesized_crt_rejects_missing_main_before_output() {
    if !have_tools() {
        return;
    }
    let Some(interpreter) = dynamic_linker() else {
...
"#,
    ...
    assert!(
        stderr.contains("CRT startup requires a defined global STT_FUNC symbol named main"),
        "{stderr}"
    );
