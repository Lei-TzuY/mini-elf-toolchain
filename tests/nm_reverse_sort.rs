...
#[test]
fn reverse_numeric_sort_matches_gnu_nm_for_real_et_rel() {
    if !tool_available("as") || !tool_available("nm") {
        return;
    }

    let dir = temp_dir("nm-reverse-sort");
    ...
    assert!(high < middle && middle < low, "{short_stdout}");
    ...
    assert!(
        gnu_high < gnu_middle && gnu_middle < gnu_low,
        "{gnu_stdout}"
    );
...
#[test]
fn reverse_sort_keeps_malformed_input_failure_atomic() {
    let dir = temp_dir("nm-reverse-sort-malformed");
