...
fn temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-ibt-plt-{label}-{}-{nonce}",
        std::process::id()
    ));
...
#[test]
fn shared_ibt_plt_matches_gnu_and_preserves_lazy_binding() {
    if !have_tools() {
        return;
    }
...
uint64_t host_function(void) {
    calls += 1;
    return UINT64_C(40) + calls;
}

int main(int argc, char **argv) {
    if (argc != 3) return 180;
    ...
    if (!call_host) return 182;

    struct link_map *map = 0;
    if (dlinfo(handle, RTLD_DI_LINKMAP, &map) != 0 || !map) return 183;
    uintptr_t *slot = (uintptr_t *)(map->l_addr + slot_offset);
    uintptr_t before = *slot;
    if (before == (uintptr_t)&host_function) return 184;

    if (call_host() != UINT64_C(41)) return 185;
    uintptr_t after_first = *slot;
    if (after_first != (uintptr_t)&host_function) return 186;

    if (call_host() != UINT64_C(42)) return 187;
    if (*slot != after_first) return 188;

    return dlclose(handle) == 0 ? 0 : 189;
}
"#,
        )
        .unwrap();
        ...
        for shared in [&mini, &gnu] {
            ...
            assert!(
                status.success(),
                "IBT PLT runtime returned {status} for {}",
                shared.display()
            );
        }
    }
