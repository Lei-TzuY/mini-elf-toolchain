...
fn temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-shared-ie-import-{label}-{}-{nonce}",
        std::process::id()
    ));
...
#[test]
fn shared_initial_exec_import_binds_provider_tls_and_preserves_thread_instances() {
    if !have_tools() {
        return;
    }
...
"#,
    ...
    assert!(input.contains("provider_tls"), "{input}");
    ...
    assert!(
        ...
        "cross-DSO initial-exec consumer must advertise DF_STATIC_TLS: {dynamic}"
    );
    ...
    assert!(
        ...
        "cross-DSO initial-exec must remain a TPOFF64 slot rather than a TLSGD descriptor: {relocations}"
    );
...
int main(int argc, char **argv) {
    ...
    if (!read_tls || !write_tls) return 102;

    if (read_tls() != UINT64_C(0x123456789abcdef0)) return 103;
    write_tls(UINT64_C(0xaaaaaaaaaaaaaaaa));
    if (read_tls() != UINT64_C(0xaaaaaaaaaaaaaaaa)) return 104;

    struct ctx ctx = { read_tls, write_tls, -1 };
    pthread_t thread;
    if (pthread_create(&thread, 0, worker, &ctx) != 0) return 105;
    if (pthread_join(thread, 0) != 0) return 106;
    if (ctx.result != 0) return ctx.result;
    if (read_tls() != UINT64_C(0xaaaaaaaaaaaaaaaa)) return 107;
...
"#,
        ...
        assert!(
            status.success(),
            "cross-DSO initial-exec TLS runner returned {status}"
        );
    }
...
#[test]
fn shared_initial_exec_provider_matching_requires_tls_symbol_type() {
    if !have_tools() {
        return;
    }
