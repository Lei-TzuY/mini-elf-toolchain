...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-cli-gotpc32-{}-{nonce}",
        std::process::id()
    ));
...
#[test]
fn cli_links_gnu_gotpc32_against_synthetic_got_base() {
    ...
    assemble(
        &dir,
        "start",
        ".globl _start\n.type _start,@function\n.extern anchor\n_start:\n  lea gotpc_value(%rip), %rsi\n  movslq gotpc_value(%rip), %rax\n  add %rsi, %rax\n  lea anchor@GOTPCREL(%rip), %rdx\n  cmp %rdx, %rax\n  jne .Lbad\n  mov $60,%rax\n  xor %rdi,%rdi\n  syscall\n.Lbad:\n  mov $60,%rax\n  mov $1,%rdi\n  syscall\n.section .data\n.align 4\ngotpc_value:\n  .long 0\n  .reloc gotpc_value, R_X86_64_GOTPC32, anchor\n",
    );
    assemble(
        &dir,
        "defs",
    ...
    assert!(
        relocation_stdout.contains("R_X86_64_GOTPC32"),
        "assembler did not produce GOTPC32 relocation: {relocation_stdout}"
    );
    ...
    #[cfg(target_os = "linux")]
    {
        let status = Command::new(dir.join("ours.out"))
            .status()
            .expect("execute linked GOTPC32 ELF");
        assert!(status.success(), "GOTPC32 executable returned {status}");
    }
