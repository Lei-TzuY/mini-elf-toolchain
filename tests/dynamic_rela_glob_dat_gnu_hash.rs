...
fn run_tool(image: &Path, bias: &str) -> Output {
    Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-glob-dat"))
        .arg("--load-bias")
        .arg(bias)
        .arg(image)
        .output()
...
#[test]
fn accepts_gnu_hash_only_glob_dat_image() {
    let dir = temp_dir("glob-dat-gnu-hash");
...
fn rejects_malformed_gnu_hash_bloom_count() {
    let dir = temp_dir("glob-dat-gnu-hash-bloom");
    let image = build_fixture(&dir);
...
fn rejects_runtime_target_overflow_with_gnu_hash() {
    let dir = temp_dir("glob-dat-gnu-hash-overflow");
    let image = build_fixture(&dir);
