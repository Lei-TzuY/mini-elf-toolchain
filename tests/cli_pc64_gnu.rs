...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-cli-pc64-{}-{nonce}",
        std::process::id()
    ));
...
#[test]
fn cli_links_gnu_r_x86_64_pc64_cross_object_reference() {
    ...
    let output = dir.join("linked-pc64");
    ...
    assert!(
        relocation_stdout.contains("R_X86_64_PC64"),
        "GNU as fixture did not contain R_X86_64_PC64: {relocation_stdout}"
    );
