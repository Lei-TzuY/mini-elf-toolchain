...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-cli-image-base-{}-{nonce}",
        std::process::id()
    ));
...
#[test]
fn cli_places_entry_at_configured_base_and_matches_gnu_text_address() {
    ...
    #[cfg(target_os = "linux")]
    {
        let status = Command::new(dir.join("custom.out"))
            .status()
            .expect("execute custom-base static ELF");
        assert!(status.success(), "custom-base executable returned {status}");
    }
    ...
    let invalid = Command::new(env!("CARGO_BIN_EXE_mini-elf-toolchain"))
        ...
        .output()
        .expect("run linker with overflowing base");
    assert_eq!(invalid.status.code(), Some(2));
    assert!(String::from_utf8_lossy(&invalid.stderr).contains("invalid image base"));
