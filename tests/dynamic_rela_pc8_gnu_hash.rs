...
fn run_tool(image: &Path) -> Output {
    Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-pc8"))
        .arg(image)
        .output()
        .unwrap()
}
...
fn accepts_gnu_hash_only_image_and_gnu_readelf_recognizes_relocation() {
    ...
    assert!(reloc_text.contains("R_X86_64_PC8"), "{reloc_text}");
    ...
    assert!(String::from_utf8_lossy(&output.stdout).contains("Validated R_X86_64_PC8 relocations"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_malformed_gnu_hash_bloom_count() {
    let dir = temp_dir("pc8-gnu-hash-bloom");
    let image = build_fixture(&dir);
