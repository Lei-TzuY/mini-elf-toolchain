...
#[test]
fn cli_links_gnu_r_x86_64_plt32_cross_object_call() {
    if !have_gnu_toolchain() {
        return;
    }
    ...
    let output = dir.join("linked-plt32");
    ...
    let relocation_stdout = String::from_utf8_lossy(&relocations.stdout);
    assert!(
        relocation_stdout.contains("R_X86_64_PLT32"),
        "GNU as fixture did not contain R_X86_64_PLT32: {relocation_stdout}"
    );
