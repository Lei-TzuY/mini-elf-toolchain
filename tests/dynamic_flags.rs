...
fn tool_available(tool: &str) -> bool { Command::new(tool).arg("--version").output().is_ok() }
...
fn build_shared(dir: &std::path::Path) -> std::path::PathBuf {
    ...
    let linked = Command::new("ld")
        .arg("-shared").arg("-z").arg("now").arg("-z").arg("origin")
        .arg("-o").arg(&shared).arg(&object).output().unwrap();
...
fn dynamic_flags_match_gnu_readelf_policy_names() {
    ...
    let gnu = Command::new("readelf").arg("-dW").arg(&shared).output().unwrap();
    assert!(gnu.status.success());
    let gnu = String::from_utf8_lossy(&gnu.stdout);
    let ours = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynflags")).arg(&shared).output().unwrap();
...
fn duplicate_dt_flags_in_later_input_keeps_stdout_atomic() {
    ...
    for offset in (dynamic_offset..dynamic_offset + dynamic_size).step_by(16) {
        ...
        if tag == 30 { first_flags = Some(offset); }
        else if tag != 0 && tag != 0x6fff_fffb && victim.is_none() { victim = Some(offset); }
    }
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynflags")).arg(&good).arg(&bad).output().unwrap();
...
fn overflowing_dynamic_file_range_is_rejected() {
    ...
    fs::write(&bad, bytes).unwrap();
    let output = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynflags")).arg(&bad).output().unwrap();
    assert!(!output.status.success());
