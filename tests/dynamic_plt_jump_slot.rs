...
fn run_tool(inputs: &[&Path], bias: &str) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_mini-elf-dynplt-jump-slot"));
...
fn jump_slot_rela_offset(bytes: &[u8]) -> usize {
    let jmprel = dynamic_tag(bytes, DT_JMPREL);
    let size = dynamic_tag(bytes, DT_PLTRELSZ);
    let mut cursor = map_vaddr(bytes, jmprel);
    ...
    panic!("missing R_X86_64_JUMP_SLOT");
}
...
#[test]
fn validates_gnu_jump_slot_for_external_symbol() {
    let dir = temp_dir("jump-slot-good");
    ...
    assert!(stdout.contains("R_X86_64_JUMP_SLOT"));
    assert!(stdout.contains(":external_fn "));
...
fn rejects_out_of_range_dynamic_symbol_index() {
    let dir = temp_dir("jump-slot-symbol-index");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
    let rela = jump_slot_rela_offset(&bytes);
    let count = symbol_count(&bytes);
    bytes[rela + 8..rela + 16]
        .copy_from_slice(&((count << 32) | u64::from(R_X86_64_JUMP_SLOT)).to_le_bytes());
...
#[test]
fn rejects_non_jump_slot_entry_in_jmprel() {
    let dir = temp_dir("jump-slot-type");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
    let rela = jump_slot_rela_offset(&bytes);
    let info = read_u64(&bytes, rela + 8);
    let symbol = info >> 32;
    bytes[rela + 8..rela + 16].copy_from_slice(&((symbol << 32) | 6).to_le_bytes());
    let bad = dir.join("bad-type.so");
    fs::write(&bad, bytes).unwrap();
    let output = run_tool(&[&bad], "0x70000000");
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains("expected R_X86_64_JUMP_SLOT"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_target_outside_writable_load() {
    let dir = temp_dir("jump-slot-target");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
    let rela = jump_slot_rela_offset(&bytes);
...
fn rejects_nonzero_rela_addend() {
    let dir = temp_dir("jump-slot-addend");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
    let rela = jump_slot_rela_offset(&bytes);
...
#[test]
fn rejects_runtime_slot_address_overflow() {
    let dir = temp_dir("jump-slot-overflow");
    let image = build_fixture(&dir);
...
fn malformed_later_input_keeps_stdout_atomic() {
    let dir = temp_dir("jump-slot-atomic");
...
#[test]
fn fixture_exposes_dynamic_symbol_table() {
    let dir = temp_dir("jump-slot-dynsym");
    let image = build_fixture(&dir);
    let bytes = fs::read(&image).unwrap();
    let symbol_index = read_u64(&bytes, jump_slot_rela_offset(&bytes) + 8) >> 32;
    let symtab = map_vaddr(&bytes, dynamic_tag(&bytes, DT_SYMTAB));
    assert!(symtab + symbol_index as usize * 24 + 24 <= bytes.len());
    fs::remove_dir_all(dir).unwrap();
}
