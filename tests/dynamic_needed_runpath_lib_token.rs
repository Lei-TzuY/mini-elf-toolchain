...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-needed-runpath-lib-token-{}-{stamp}",
        std::process::id()
    ));
...
#[test]
fn expands_x86_64_lib_token_as_safe_runpath_component() {
    let dir = temp_dir();
    let root_dir = dir.join("root");
    let plugins = root_dir.join("lib64").join("plugins");
    ...
    assert!(dynamic.contains("${ORIGIN}/${LIB}/plugins"));
...
#[test]
fn rejects_lib_token_embedded_inside_a_path_component() {
    let dir = temp_dir();
    let root_dir = dir.join("root");
    let fallback = dir.join("fallback");
    fs::create_dir_all(&fallback).unwrap();
