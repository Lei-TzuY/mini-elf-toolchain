...
fn temp_dir() -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!(
        "mini-elf-toolchain-cli-got32-{}-{nonce}",
        std::process::id()
    ));
...
#[test]
fn cli_builds_static_got_offsets_for_gnu_got32() {
    ...
    assemble(
        &dir,
        "start",
        ".globl _start\n.type _start,@function\n.extern alpha\n.extern helper\n_start:\n  cmpl $8, helper_got_offset(%rip)\n  jne .Lbad\n  cmpl $0, alpha_got_offset(%rip)\n  jne .Lbad\n  mov $60,%rax\n  xor %rdi,%rdi\n  syscall\n.Lbad:\n  mov $60,%rax\n  mov $1,%rdi\n  syscall\n.section .data\n.align 4\nhelper_got_offset:\n  .long 0\n  .reloc helper_got_offset, R_X86_64_GOT32, helper\nalpha_got_offset:\n  .long 0\n  .reloc alpha_got_offset, R_X86_64_GOT32, alpha\n",
    );
    assemble(
        &dir,
        "defs",
    ...
    assert!(
        relocation_stdout.matches("R_X86_64_GOT32").count() >= 2,
        "assembler did not produce both GOT32 relocations: {relocation_stdout}"
    );
    ...
    #[cfg(target_os = "linux")]
    {
        let status = Command::new(dir.join("ours.out"))
            .status()
            .expect("execute linked GOT32 ELF");
        assert!(status.success(), "GOT32 executable returned {status}");
    }
