...
fn run_tool(image: &Path) -> Output {
    Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-pc16"))
        .arg(image)
        .output()
        .unwrap()
}
...
fn accepts_gnu_hash_only_image_and_gnu_readelf_recognizes_relocation() {
    let dir = temp_dir("pc16-gnu-hash");
    ...
    assert!(reloc_text.contains("R_X86_64_PC16"), "{reloc_text}");
    ...
    assert!(String::from_utf8_lossy(&output.stdout)
        .contains("Validated R_X86_64_PC16 relocations"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_malformed_gnu_hash_bloom_count() {
    let dir = temp_dir("pc16-gnu-hash-bloom");
    let image = build_fixture(&dir);
