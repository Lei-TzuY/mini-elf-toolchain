...
fn run_tool(inputs: &[&Path]) -> Output {
    let mut command = Command::new(env!("CARGO_BIN_EXE_mini-elf-eh-frame-personality"));
    for input in inputs {
        command.arg(input);
    }
...
fn zpr_payload(file: &[u8]) -> (usize, usize) {
    ...
    assert_eq!(&file[aug_start..aug_start + nul], b"zPR");
...
fn readelf_personality_symbol(image: &Path) -> u64 {
    ...
    for line in text.lines() {
        ...
        if fields.last() == Some(&"personality") && fields.len() >= 2 {
            return u64::from_str_radix(fields[1], 16).unwrap();
        }
    }
    panic!("GNU readelf did not report personality symbol");
}

#[test]
fn validates_gnu_zpr_personality_against_readelf_symbol() {
    let dir = temp_dir("eh-personality-good");
    ...
    let personality = readelf_personality_symbol(&image);
    ...
    assert!(frames.lines().any(|line| {
        line.trim_start()
            ...
            .is_some_and(|value| value.trim() == "\"zPR\"")
    }));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_unsupported_personality_encoding() {
    let dir = temp_dir("eh-personality-encoding");
    let image = build_fixture(&dir);
    let mut file = fs::read(&image).unwrap();
    let (_, payload) = zpr_payload(&file);
    file[payload] = 0;
    ...
    assert!(String::from_utf8_lossy(&output.stderr)
        .contains("personality encoding is not pcrel/sdata4"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_personality_pointer_arithmetic_underflow() {
    let dir = temp_dir("eh-personality-underflow");
    let image = build_fixture(&dir);
    let mut file = fs::read(&image).unwrap();
    let (_, payload) = zpr_payload(&file);
    file[payload + 1..payload + 5].copy_from_slice(&i32::MIN.to_le_bytes());
    ...
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr)
        .contains("personality pointer arithmetic overflows u64"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_personality_target_outside_executable_load() {
    let dir = temp_dir("eh-personality-nonexec");
    let image = build_fixture(&dir);
    let mut file = fs::read(&image).unwrap();
    let (_, payload) = zpr_payload(&file);
    file[payload + 1..payload + 5].copy_from_slice(&0_i32.to_le_bytes());
    let malformed = dir.join("nonexec.so");
    fs::write(&malformed, file).unwrap();

    let output = run_tool(&[&malformed]);
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr)
        .contains("is not executable file-backed PT_LOAD data"));
    fs::remove_dir_all(dir).unwrap();
}

#[test]
fn rejects_malformed_zpr_payload_length() {
    let dir = temp_dir("eh-personality-length");
    let image = build_fixture(&dir);
    let mut file = fs::read(&image).unwrap();
    let (length_offset, _) = zpr_payload(&file);
...
fn multi_input_failure_keeps_stdout_atomic() {
    let dir = temp_dir("eh-personality-atomic");
