...
fn rela_of_type(bytes: &[u8], kind: u32) -> usize {
    let mut cursor = map_vaddr(bytes, dynamic_tag(bytes, DT_RELA).unwrap());
    loop {
        if read_u64(bytes, cursor + 8) as u32 == kind {
            return cursor;
        }
        cursor += 24;
    }
...
fn build_fixture(dir: &Path) -> PathBuf {
    ...
    let rela = rela_of_type(&bytes, R_X86_64_DTPOFF64);
    let info = read_u64(&bytes, rela + 8);
    bytes[rela + 8..rela + 16]
        .copy_from_slice(&(((info >> 32) << 32) | u64::from(R_X86_64_DTPOFF32)).to_le_bytes());
    fs::write(&image, bytes).unwrap();
    image
}

fn run_tool(image: &Path) -> Output {
    Command::new(env!("CARGO_BIN_EXE_mini-elf-dynrela-dtpoff32"))
...
fn accepts_gnu_hash_only_image_and_gnu_readelf_recognizes_relocation() {
    let dir = temp_dir("dtpoff32-gnu-hash");
    ...
    assert!(reloc_text.contains("R_X86_64_DTPOFF32"), "{reloc_text}");
...
fn rejects_malformed_gnu_hash_bloom_count() {
    let dir = temp_dir("dtpoff32-gnu-hash-bloom");
    let image = build_fixture(&dir);
    let mut bytes = fs::read(&image).unwrap();
