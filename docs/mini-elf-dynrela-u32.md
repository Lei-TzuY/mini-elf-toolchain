...
The validator reads a checked `PT_DYNAMIC`, requires the ELF64 `DT_RELA` tuple plus `DT_SYMTAB`, `DT_SYMENT`, `DT_STRTAB`, and `DT_STRSZ`, and derives a bounded dynamic-symbol count from SysV `DT_HASH.nchain` when present. GNU-hash-only images are also accepted by validating `DT_GNU_HASH` Bloom, bucket, and chain metadata and deriving the highest reachable dynamic-symbol index. SysV `DT_HASH` retains precedence when both hash styles are present. For each `R_X86_64_32` entry it requires a nonzero in-range dynamic symbol index, a four-byte relocation destination fully contained in writable `PT_LOAD` memory, and a same-image defined non-absolute, non-TLS symbol whose value lies in loadable memory. Symbol names must terminate inside the declared dynamic string table.
...
Focused integration coverage builds real GNU `as`/`ld -shared` fixtures for SysV and GNU hash styles, preserves the GNU-produced ET_DYN/DT_RELA/dynsym/load layout, changes only the selected same-image data relocation type to `R_X86_64_32`, and confirms GNU `readelf -dW/-rW` recognizes the GNU-hash-only image and relocation. Regressions cover malformed GNU-hash Bloom metadata, invalid dynamic-symbol indices, non-writable targets, unsigned-32 overflow, signed addends, and multi-input stdout atomicity.
