...
`mini-elf-gnu-property` inspects the ELF64 x86-64 `PT_GNU_PROPERTY` program-header segment and decodes a bounded GNU x86 policy subset carried by `GNU_PROPERTY_X86_FEATURE_1_AND` and `GNU_PROPERTY_X86_ISA_1_NEEDED`.
...
Recognized x86 ISA-needed levels are:
...
Unknown bits in either recognized mask are preserved numerically rather than silently discarded.
