...
The focused regression suite builds a real GNU `as` / `ld -shared --hash-style=sysv` fixture containing `R_X86_64_PC16`, confirms it with GNU `readelf -rW`, and covers invalid symbol indexes, non-writable targets, signed-16 overflow, a negative same-image result, and multi-input stdout atomicity.
